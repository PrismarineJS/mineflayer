...
var block_finder = function() {
     ...
     * @param {ItemType} OR {Array} OR {function(block) -> bool} matching An ItemType to look for, OR an array of ItemTypes to look for, OR a comparison function
     ...
     */
    _public.findNearest = function(point, matching, radius, count) {
        ...
        if (typeof matching === "number") {
            matches = function(block) {
                return matching === block.type;
            }
        } else if (typeof matching === "function") {
            matches = matching;
        } else if (matching.length !== undefined && matching.length > 0 && typeof matching.length !== "function") {
            // Assuming list
            ...
            matches = function(block) {
                return matching_set.contains(block.type);
            }
        } else {
            return [];
        }
                ...
                for (pt.x = 0; pt.x <= max.x; pt.x++) {
                    for (pt.y = 0; pt.y <= max.y; pt.y++) {
                        for (pt.z = 0; pt.z <= max.z; pt.z++) {
                            ...
                            if (matches(mf.blockAt(abs_coords))) {
