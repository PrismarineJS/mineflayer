...
function inject (bot) {
  ...
  async function placeEntityWithOptions (referenceBlock, faceVector, options) {
    ...
    assert(['end_crystal', 'boat', 'spawn_egg', 'armor_stand', 'item_frame'].includes(type), 'Unimplemented')
  ...
  function waitForEntitySpawn (name, placePosition) {
    ...
    if (name === 'end_crystal') {
      ...
      mobName = bot.supportFeature('entityNameUpperCaseNoUnderscore') ? 'ItemFrame' : 'item_frame'
    }
