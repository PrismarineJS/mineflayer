...
function inject (bot, { version }) {
  ...
  function waitForEntitySpawn (name, placePosition) {
    ...
    if (name === 'end_crystal') {
      ...
      mobName = bot.supportFeature('entityNameUpperCaseNoUnderscore') ? 'ArmorStand' : 'armor_stand'
    }

    return new Promise((resolve, reject) => {
      function listener (entity) {
        const dist = entity.position.distanceTo(placePosition)
        console.log(`${entity.name} spawned, expecting ${mobName}, distance: ${dist}`)
        console.log(`${entity.name === mobName ? 'Names are equal!' : "Names aren't equal?"}`)
        console.log(`${dist < maxDistance ? 'Distance is good!' : "Distance isn't good?"}`)
