  ...
  async function craftOnce (recipe, craftingTable) {
    ...
    async function startClicking (window, w, h) {
      ...
      async function grabResult () {
        ...
        // Use the server's actual result from the window slot, not the
        // recipe data — the server may use different metadata than what
        // the recipe says (e.g. pre-1.13 wildcard metadata).
        // The server updates slot 0 via set_slot after ingredients are placed.
        if (window.slots[0]) {
          // Server already populated the result — use it as-is
        } else {
          // Fallback: set from recipe (pre-1.13 servers may not update)
