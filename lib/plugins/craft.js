...
module.exports = (bot, { version }) => {
  ...
  const getNonEmptyCraftingSlots = (window) => window.slots.filter(item => item?.slot >= 1 && item?.slot <= getWindowWidth() ** 2) // get all items from slot 1 to 4 || 9 depending on whether we are using a crafting table
  const getSlotFromXY = (x, y, max) => +x + 1 + max * +y
  ...
  async function craft (recipe, count = 1, craftingTable) {
    ...
    const craftingWindow = recipe.requiresTable ? await bot.openBlock(craftingTable) : true
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (!bot.supportFeature('craftingDoesSetSlot')) await updateCraftingSlots(recipe)
    ...
    if (recipe.inShape) {
      ...
      for (const [y, row] of Object.entries(recipeParts)) {
        for (const [x, item] of Object.entries(row)) {
          await putItemInSlot(item, getSlotFromXY(x, y, max))
        }
      }
    } else {
      let y = 0
  ...
  async function putItemInSlot (itemNeeded, slot) {
    ...
    if (!window.selectedItem || window.selectedItem.type !== itemNeeded.id) {
  ...
  async function clearCraftingWindow () {
    const window = getCurrentWindow()
    const slotsToClear = getNonEmptyCraftingSlots(window)
  ...
  async function updateCraftingSlots (recipe) {
    const window = getCurrentWindow()
    const slotsToClear = getNonEmptyCraftingSlots(window).map(item => item.slot)
    if (recipe.outShape) {
      const max = getWindowWidth()
      for (const [y, row] of Object.entries(recipe.outShape)) {
        for (const [x, outItem] of Object.entries(row)) {
          const newItem = outItem.id !== -1 ? new Item(outItem.id, outItem.count, outItem.metadata) : null
          window.updateSlot(getSlotFromXY(x, y, max), newItem)
        }
      }
    } else {
      slotsToClear.forEach(slot => window.updateSlot(slot, null))
    }
  }
}
