  ...
  async function craftOnce (recipe, craftingTable) {
    ...
    async function startClicking (window, w, h) {
      ...
      if (recipe.inShape) {
        ...
        const clickPromises = []
        clickPromises.push(clickShape(it.x, it.y, it.row))
        while (incrementShapeIterator()) {
          clickPromises.push(clickShape(it.x, it.y, it.row))
        }
      ...
      async function clickShape (x, y, row) {
        const promiseArray = []
        const destSlot = slot(x, y)
        const ingredientId = row[x]
        ...
        if (!window.selectedItem || window.selectedItem.type !== ingredientId) {
          ...
          const sourceItem = window.findInventoryItem(ingredientId)
          if (!sourceItem) throw new Error('missing ingredient')
          if (originalSourceSlot == null) originalSourceSlot = sourceItem.slot
          promiseArray.push(bot.clickWindow(sourceItem.slot, 0, 0))
        }
        promiseArray.push(bot.clickWindow(destSlot, 1, 0))
        await Promise.all(promiseArray)
      }

      async function nextIngredientsClick () {
        const ingredientId = recipe.ingredients[ingredientIndex]
      ...
      async function updateOutShape () {
        ...
        for (const _slot of slotsToClick) {
          promiseArray.push(bot.putAway(_slot))
        }
        await Promise.all(promiseArray)
      }
