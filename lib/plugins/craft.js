...
function inject (bot) {
  ...
  async function craftBatch (recipes, craftingTable) {
    if (!Array.isArray(recipes)) throw new Error('recipes value must be array of recipe with count object')

    try {
      for (let i = 0; i < recipes.length; i++) {
        const { recipe, count } = recipes[i]
