...
function inject (bot, options) {
  ...
  const sendBrand = () => bot._client.writeChannel(brandChannel, options.brand)
  if (bot.supportFeature('hasConfigurationState')) {
    // the vanilla client announces its brand as soon as it enters the configuration state
    // (before its client settings), and again every time the server sends it back there
    bot._client.on('state', (newState) => {
      if (newState === 'configuration') sendBrand()
    })
  }
  ...
  bot._client.on('login', (packet) => {
    ...
    if (!bot.supportFeature('hasConfigurationState')) sendBrand()
  })
