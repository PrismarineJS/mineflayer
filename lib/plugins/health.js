...
function inject (bot) {
  ...
  bot._client.on('combat_event', (eventData) => {
    ...
    if (eventData.message.with.length > 1) {
      ...
      // return
      if (thisWith.hoverEvent.hasOwnProperty('contents')) {
        if (thisWith.hoverEvent.contents.type === 'minecraft:player') deathBy = thisWith.text
        else deathBy = thisWith.hoverEvent.contents.type
      } else {
        ...
        if (thisWith.hoverEvent.value.text.type === 'minecraft:player' || !thisWith.hoverEvent.value.text.hasOwnProperty('type')) deathBy = thisWith.text
