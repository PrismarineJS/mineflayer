...
function inject (bot) {
  ...
  bot._client.on('combat_event', (eventData) => {
    ...
    if (eventData.message.with.length > 1) {
      const thisWith = eventData.message.with[1]
      // console.log(thisWith)
      // return
      if (Object.prototype.hasOwnProperty.call(thisWith.hoverEvent, 'contents')) {
        if (thisWith.hoverEvent.contents.type === 'minecraft:player') deathBy = thisWith.text
        else deathBy = thisWith.hoverEvent.contents.type
      } else {
        thisWith.hoverEvent.value.text = thisWith.hoverEvent.value.text.replace(/([{,]\s*)(\w+)(\s*:)/g, '$1"$2"$3')
        thisWith.hoverEvent.value.text = thisWith.hoverEvent.value.text.replaceAll('\'', '')
        thisWith.hoverEvent.value.text = JSON.parse(thisWith.hoverEvent.value.text)
        if (thisWith.hoverEvent.value.text.type === 'minecraft:player' || !Object.prototype.hasOwnProperty.call(thisWith.hoverEvent.value.text, 'type')) deathBy = thisWith.text
        else deathBy = thisWith.hoverEvent.value.text.type
      }
    }
    bot.emit('death', deathType, deathBy)
  })
