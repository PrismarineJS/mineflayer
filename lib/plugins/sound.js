...
function inject (bot) {
  ...
  bot._client.on('sound_effect', (packet) => {
    ...
    console.log('Sound effect heard:', {
      soundName,
      soundId,
      soundCategory,
      position: pt,
    ...
    // Now, emit the best event we can based on what we found
    if (soundName) {
      bot.emit('soundEffectHeard', soundName, pt, volume, pitch)
    } else if (soundId !== null) {
      // Fallback for sounds that are truly only identified by an ID
      // (e.g., custom sounds on a server not in the registry)
