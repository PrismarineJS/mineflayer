...
const { Task, ErrorTaskAlreadyRunning, ErrorTaskConflict } = require(path.join(__dirname, '../taskUtil'))
...
function inject (bot) {
  ...
  function taskCompatible (taskIdentifier) {
  ...
  function taskCreateCompatible (taskIdentifier) {
    ...
    const { status, task: incompatiblyTask } = taskCompatible(taskIdentifier)
    if (status === false) {
  ...
  async function taskWaitCompatibility (taskIdentifier) {
    const { status } = taskCompatible(taskIdentifier)
  ...
  bot.taskCompatible = taskCompatible
