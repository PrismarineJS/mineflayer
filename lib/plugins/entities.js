...
function inject(bot,{version}) {
  ...
  bot.findPlayer = bot.findPlayers = function(filter) {
    const filterFn = function(entity) {
  ...
  bot._client.on('remove_entity_effect', function(packet) {
    ...
    const entity = fetchEntity(packet.entityId);
    const effect = entity.effects[packet.effectId];
    if(effect) {
  ...
  bot._client.on('player_info', function(packet) {
    ...
    packet.data.forEach(function(item) {
      ...
      const player = bot.uuidToUsername[item.UUID] ? bot.players[bot.uuidToUsername[item.UUID]] : null;
      if(packet.action === 0) {
        // New Player
        if(!player) {
  ...
  bot._client.on('set_passengers',function({entityId,passengers}) {
