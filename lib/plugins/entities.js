...
function inject (bot) {
  ...
  // Store entities in prismarine-world for centralized world state
  bot.saveEntityToWorld = (entity) => {
    if (entity.uuid) {
      bot.world.setEntity(entity.uuid, {
        id: entity.id,
        type: entity.type,
  ...
  bot.loadEntityFromWorld = (uuid) => {
    return bot.world.getEntity(uuid)
  }
  ...
  bot._client.on('entity_destroy', (packet) => {
    ...
    packet.entityIds.forEach((id) => {
      ...
      // Remove from world storage too
      if (entity.uuid) {
        bot.world.removeEntity(entity.uuid)
      }
  ...
  function fetchEntity (id) {
    if (!bot.entities[id]) {
      bot.entities[id] = new Entity(id)
      // Also save new entities to world storage for centralized state
      if (bot.entities[id].uuid) {
        bot.saveEntityToWorld(bot.entities[id])
      }
    }
