...
function inject (bot) {
  ...
  function extractSkinInformation (properties) {
    ...
    const props = Object.fromEntries(properties.map((e) => bot.supportFeature('skinUsesKeyedProperties') ? [e.key, e] : [e.name, e]))
    if (!props.textures || !props.textures.value) {
      return undefined
    }
    const decodedString = Buffer.from(props.textures.value, 'base64').toString('utf8')
    const correctedString = decodedString.replace(/([{,]\s*)([a-zA-Z0-9_]+)\s*:/g, '$1"$2":')
    const skinTexture = JSON.parse(correctedString)
