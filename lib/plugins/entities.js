...
function inject (bot) {
  ...
  // `steps` array (see the `entityDelta` protodef type in node-minecraft-protocol): normally
  // a single { dX, dY, dZ, ticks: 0 } step (the old flat format), but when the server batched
  // several ticks' worth of movement into one packet it's several chained steps, each relative
  // to the position produced by the previous one. Applying them in order (rather than only the
  // first/last) is required to end up at the right final position -- dropping steps causes
  // entities to visibly drift/jump the more ticks get batched. Pre-26.3 protocol versions never
  // have `packet.steps`; they still carry the flat dX/dY/dZ fields directly, so normalize both
  // shapes down to a single steps array here.
  function entityDeltaSteps (packet) {
    if (packet.steps) return packet.steps
