...
function inject (bot) {
  ...
  function setElytraFlyingState(entity, elytraFlying) {
    entity.elytraFlying = elytraFlying
  ...
  const fireworksItemName = bot.registry.itemsArray.find(item => item.displayName === "Firework Rocket")?.name
  bot._client.on('entity_metadata', (packet) => {
    ...
    if (bot.supportFeature('mcDataHasEntityMetadata')) {
      ...
      if (fireworksItemName === entity.name) {
        if (metas.attached_to_target !== undefined && metas.attached_to_target !== 0) {
      ...
      if (fireworksItemName === entity.name) {
        const attachedToTarget = packet.metadata.find(e => e.key === 9)
        if (attachedToTarget !== undefined && attachedToTarget.value !== 0) {
          const entityId = attachedToTarget.value - 1
          if (entityId === bot.entity?.id) {
            const fireworksItem = packet.metadata.find(e => e.key === 8)
