...
function inject (bot) {
  ...
  bot._client.on('set_passengers', ({ entityId, passengers }) => {
    ...
    // set_passengers repeats this function every time the bot spawns in a world. need to check vehicle.type to validate vehicle.
    if (!vehicle.type) return

    // Nothing has changed. The bot repeats this event twice due to a vehicle being already in the world while spawning.
    if (vehicle.passengers.length === passengers.length) return

    // An entity has joined, there is no problem if its a player or not.
    if (passengers.length === 1 && vehicle.passengers.length === 0) {
      vehicle.passengers = passengers
      entity = fetchEntity(passengers[0])
      bot.emit('entityAttach', vehicle, entity)
      return
    }

    // An entity has joined the vehicle.
    if (passengers.length === 2 && vehicle.passengers.length === 1) {
      // The leader of the vehicle is a player. and an entity is with them. players automatically get assigned the leader if an entity is inside.
      // Entities cannot enter the bot if a player is inside the boat. (Tested)
      // No need to check if its an entity or a player.
      if (fetchEntity(passengers[0]).username && !fetchEntity(passengers[1]).username) {
        vehicle.passengers = passengers
      ...
      // Other possibilities , [entity, entity] and [player, player] will have no issues.
      entity = fetchEntity(passengers[1])
      vehicle.passengers = passengers
      bot.emit('entityAttach', vehicle, entity)
      return
    }

    // An entity has left the vehicle.
    if (passengers.length === 0 && vehicle.passengers.length === 1) {
      entity = fetchEntity(vehicle.passengers[0])
      vehicle.passengers = []

      bot.emit('entityDetach', vehicle, entity)
      return
    }

    // We need to repeat (vehicle.passengers), since we need to get the old entity that was on the vehicle.
    if (passengers.length === 1 && vehicle.passengers.length === 2) {
      // a player was on the boat.
      if (fetchEntity(vehicle.passengers[0]).username && !fetchEntity(vehicle.passengers[1]).username) {
        // The player is still on the boat.
        if (passengers.includes(vehicle.passengers[0])) {
          entity = fetchEntity(vehicle.passengers[1])
          vehicle.passengers = passengers
