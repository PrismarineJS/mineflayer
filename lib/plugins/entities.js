...
function inject (bot, { version }) {
  ...
  bot._client.on('entity_destroy', (packet) => {
    // destroy entity
    const entityIds = packet.entityIds ?? [packet.entityId]
  ...
  // TEMP: Misnamed packet in 1.17 protocol, entityIds is a single varint
  bot._client.on('destroy_entity', (packet) => {
    bot._client.emit('entity_destroy', { entityId: packet.entityIds })
  })
