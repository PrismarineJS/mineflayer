      ...
      if (packet.action === 0) {
        ...
        if (!player) {
          player = bot.players[item.name] = {
            ...
            get nameOnTablist () {
              const team = bot.teamMap[player.username]
              if (!team) {
                return player.displayName.clone()
              }
              if (team.nameTagVisibility === 'never') {
                return null
              }
              let name = null
              if (team.prefix) {
                name = team.prefix.clone()
                name.append(player.displayName.clone())
              } else {
                name = player.displayName.clone()
              }

              if (team.suffix) {
                if (name) {
                  name.append(team.suffix.clone())
                } else {
                  name = team.suffix.clone()
                }
              }
              return name
            }
