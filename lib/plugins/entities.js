...
function inject (bot) {
  ...
  // Vanilla Attributes.ENTITY_INTERACTION_RANGE / BLOCK_INTERACTION_RANGE: default, then the
  // bounds the effective value is clamped to. From 1.20.5 the server sends the base value and
  // modifiers (creative adds to both), which updateAttributes stores under the codec's key.
  const ENTITY_INTERACTION_RANGE = { key: 'player.entity_interaction_range', default: 3.0, min: 0, max: 64 }
  const BLOCK_INTERACTION_RANGE = { key: 'player.block_interaction_range', default: 4.5, min: 0, max: 64 }

  function interactionRange ({ key, default: fallback, min, max }) {
    const attribute = bot.entity?.attributes?.[key]
    if (!attribute) return fallback
    return Math.min(Math.max(getAttributeValue(attribute), min), max)
  }

  bot.entityInteractionRange = () => interactionRange(ENTITY_INTERACTION_RANGE)
  bot.blockInteractionRange = () => interactionRange(BLOCK_INTERACTION_RANGE)
