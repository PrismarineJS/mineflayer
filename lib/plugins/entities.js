...
function inject(bot) {
  ...
  // !!! BREAKING CHANGE !!!
  // bot.players is a lookup function for finding a player by a filter
  // If the filter is nullsy, it will return all the players.
  // Player ping and other information is now stored on the entity
  // entity.playerInfo is the property that holds ping and other information
  bot.players = function(filter) {
    ...
    // One player was expected, and one player was found.
    if(typeof filter === 'string' && resultSet.length == 1) {
      return resultSet[0];
    }
    return resultSet;
  }
  ...
  bot.client.on('player_info', function(packet) {
    // player list item
    console.log('PLAYER INFO PACKET', packet);
    var player = bot.players(packet.playerName);
    if(!player) {
      console.warn('Player_Info for ', packet.playerName, '(missing)');
      return;
    }
    if(packet.online) {
      // New Player
      if(!player.playerInfo) {
        player.playerInfo = { ping: packet.ping };
        bot.emit('playerJoined', player);
        // Just an Update
      } else player.playerInfo.ping = packet.ping;
    // Player has parted
    } else {
