...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  bot._client.on('position', (packet) => {
    ...
    // After a respawn, delay the position confirmation to give the server
    // time to fully process the respawn. Sending position_look too quickly
    // causes "Invalid move player packet received" kicks on older servers.
    if (justRespawned) {
      justRespawned = false
      setTimeout(() => {
