...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  // minecraft-data releases without the feature: the shift actions exist while actionId is a bare varint.
  ...
  function tickPhysics (now) {
    ...
    // Runs with physics disabled too: sendInputPackets reports the sprinting state it derives.
    updateSprinting()
  ...
  // login and respawn give the server a new player with no keys held and no sprint, so the
  // held controls are sent again.
  function resetSentInputs () {
    lastSentSneak = false
