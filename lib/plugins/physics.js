...
const { taskCreateDone, tasks } = require('../taskUtil')
...
function inject (bot, { physicsEnabled }) {
  ...
  let lookingTask = taskCreateDone()
  ...
  bot.look = async (yaw, pitch, force) => {
    if (!lookingTask.done) {
      lookingTask.cancel() // cancel the previous one
    }
    lookingTask = bot.taskCreateCompatible(tasks.physics_look)
