...
function inject (bot, { physicsEnabled }) {
  ...
  let lookingAction = actionCreateDone()

  bot.on('move', () => {
    if (!lookingAction.done && Math.abs(deltaYaw(bot.entity.yaw, lastSentYaw)) < 0.001) {
      lookingAction.finish()
    }
  })
  ...
  bot.look = async (yaw, pitch, force) => {
    if (!lookingAction.done) {
      lookingAction.cancel() // cancel the previous one
    }
    lookingAction = bot.actionCreateCompatible(actions.physics_look)
    ...
    if (force) {
      ...
      lastSentPitch = pitch
      lookingAction.finish()
      return lookingAction.promise
    }

    await lookingAction.promise
  }
