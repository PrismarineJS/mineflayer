...
function inject (bot) {
  ...
  function liquidPush (pos) {
    const flow = liquidFlow(pos)
    const push = flow.clone().scaled(physics.liquidPush)
  ...
  function liquidFlow (pos) {
    ...
    const Blocks = {
      'x+': bot.blockAt(pos.offset(1, 0, 0)),
      'x-': bot.blockAt(pos.offset(-1, 0, 0)),
      'z+': bot.blockAt(pos.offset(0, 0, 1)),
      'z-': bot.blockAt(pos.offset(0, 0, -1))
    }
    const waterBlocks = {}
    ...
    if (waterBlocks['x+'] !== null && waterBlocks['x+'] > base) flow = flow.add(1, 0, 0)
    if (waterBlocks['x-'] !== null && waterBlocks['x-'] > base) flow = flow.add(-1, 0, 0)
    if (waterBlocks['z+'] !== null && waterBlocks['z+'] > base) flow = flow.add(0, 0, 1)
    if (waterBlocks['z-'] !== null && waterBlocks['z-'] > base) flow = flow.add(0, 0, -1)
    if (waterBlocks['x+'] !== null && waterBlocks['x+'] < base && flow.x == 0) flow = flow.add(-1, 0, 0)
    if (waterBlocks['x-'] !== null && waterBlocks['x-'] < base && flow.x == 0) flow = flow.add(1, 0, 0)
    if (waterBlocks['z+'] !== null && waterBlocks['z+'] < base && flow.z == 0) flow = flow.add(0, 0, -1)
    if (waterBlocks['z-'] !== null && waterBlocks['z-'] < base && flow.z == 0) flow = flow.add(0, 0, 1)
    return flow
  }
