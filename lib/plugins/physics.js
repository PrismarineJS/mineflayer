...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  bot.on('move', () => {
    if (!lookingTask.done &&
      Math.abs(deltaYaw(lastSentYaw, lookingTask.yaw)) < 0.001 &&
      Math.abs(lastSentPitch - lookingTask.pitch) < 0.001) {
      lookingTask.finish()
    }
  })
  ...
  bot.look = async (yaw, pitch, force) => {
    if (!lookingTask.done) {
      lookingTask.cancel(new Error('bot.look was overwritten by a newer bot.look/bot.lookAt call before it synced.'))
    }
    ...
    lookingTask.yaw = bot.entity.yaw
    lookingTask.pitch = bot.entity.pitch
