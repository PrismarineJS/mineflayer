...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  bot._client.on('position', (packet) => {
    ...
    // After a respawn, skip position_look responses for a short window.
    // Sending position_look too quickly after respawn causes
    // "Invalid move player packet received" kicks on older servers.
    if (respawnTimer > 0 && Date.now() - respawnTimer < 500) {
