...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  bot._client.on('position', (packet) => {
    ...
    // After a respawn, skip the immediate position_look echo.
    // Sending it too quickly causes "Invalid move player packet received"
    // kicks on older servers. The next physics tick will send the position
    // naturally once shouldUsePhysics is set to true.
    if (justRespawned) {
      justRespawned = false
      // Update internal tracking without sending a packet
      lastSent.x = pos.x
      lastSent.y = pos.y
      lastSent.z = pos.z
      lastSent.yaw = newYaw
      lastSent.pitch = newPitch
  ...
  bot.on('respawn', () => { shouldUsePhysics = false; justRespawned = true })
  bot.on('login', () => {
