...
const PHYSICS_SETINTERVAL_MS = 10 // this runs the doPhysics() loop faster, not tickPhysics()
const PHYSICS_CATCHUP_TICKS = 4 * PHYSICS_TIMESTEP // limit number of ticks to catchup if loop gets behind
...
function inject (bot, { physicsEnabled }) {
  ...
  function doPhysics () {
    ...
    let deltaSeconds = (now - lastPhysicsFrameTime) / 1000
    lastPhysicsFrameTime = now

    if (deltaSeconds >= PHYSICS_CATCHUP_TICKS) deltaSeconds = PHYSICS_CATCHUP_TICKS // limiter (limit catchups)
  ...
  let deprecatedEvent = false // remove when physicTick is removed
  function tickPhysics (now) {
    ...
    if (bot.physicsEnabled && shouldUsePhysics) {
      ...
      if (bot.listenerCount('physicTick') > 0 && deprecatedEvent === false) { // remove when physicTick is removed
        console.log('physicTick event is deprecated! Please use physicsTick instead')
        deprecatedEvent = true
      }
    }
    if (shouldUsePhysics) {
      updatePosition(now)
  ...
  function updatePosition (now) {
    ...
    if (positionUpdated) lastSentPosition(position, now) // set lastSent.x, lastSent.y, lastSent.z and lastSent.time
    if (lookUpdated) lastSentLook(yaw, pitch) // set lastSent.yaw and lastSent.pitch
  ...
  bot.on('login', () => {
    ...
    if (doPhysicsTimer === null) {
      ...
      doPhysicsTimer = setInterval(doPhysics, PHYSICS_SETINTERVAL_MS)
    }
  })
  bot.on('end', cleanup)
}
