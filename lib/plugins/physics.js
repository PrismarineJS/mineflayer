...
function inject (bot, { physicsEnabled, maxCatchupTicks, sendClientTickEnd }) {
  ...
  // minecraft-data does not expose this feature on every 1.21.2-1.21.4
  // protocol, although those clients must terminate each movement tick.
  const supportsClientTickEnd = bot.supportFeature('sendsClientTickEndPacket') || bot.registry.version['>=']('1.21.2')
  bot.sendClientTickEnd = sendClientTickEnd ?? supportsClientTickEnd
  // Surface cheat toggles from the factory so the rest of the codebase can
  // branch on a single namespaced object. Defaults match DaPrint's "off"
  // policy: the conservative placement path stays active.
  bot.cheats = options.cheats ?? {
    mode: 'off',
    rotationPreLook: false,
    rawBlockPlace: false,
    airPlace: false,
  }
  ...
  // Env-gated Grim movement tracer. Inert unless MF_TRACE_FILE is set.
  // Writes one JSON line per event so a send-side trace can be correlated against
  // Grim's server-side actualMovement/offset without rebuilding the server.
  const traceFile = process.env.MF_TRACE_FILE
  let traceTick = 0
  const traceFs = traceFile ? require('fs') : null
  function trace (event, extra) {
    if (!traceFs) return
    try {
      const p = bot.entity?.position
      const v = bot.entity?.velocity
      traceFs.appendFileSync(traceFile, JSON.stringify({
        t: Date.now(),
        tick: traceTick,
        ev: event,
        x: p?.x, y: p?.y, z: p?.z,
    ...
    } catch { /* never let tracing break movement */ }
  }
  bot._grimTrace = trace

  // Temporary Grim-transaction ordering probe (env-gated with the tracer).
  // Grim anchors its movement clock to clientbound Ping (play) transactions;
  // if the pong/position ordering is perturbed by the proxy's event-loop hop,
  // movements get attributed to the wrong tick. Log ping in / pong+pos out.
  if (traceFs) {
    bot._client.on('ping', (data) => { trace('recvPing', { pingId: data?.id }) })
    const _origWrite = bot._client.write.bind(bot._client)
    bot._client.write = function (name, params) {
      if (name === 'pong') trace('sendPong', { pongId: params?.id })
      return _origWrite(name, params)
    }
    // Count inbound packet types for 1 window to compare proxy vs direct traffic.
    const _pktCounts = {}
    bot._client.on('packet', (data, meta) => {
      if (!meta || !meta.name) return
      _pktCounts[meta.name] = (_pktCounts[meta.name] || 0) + 1
      if (meta.name === 'update_health') {
        trace('recvHealth', { health: data?.health, food: data?.food, sat: data?.foodSaturation })
      }
    })
    setInterval(() => {
      trace('pktCounts', { counts: { ..._pktCounts } })
      for (const k in _pktCounts) delete _pktCounts[k]
    }, 5000)
  }
  ...
  function tickPhysics (now) {
    try {
      ...
      if (bot.physicsEnabled && shouldUsePhysics) {
        ...
        trace('preSim', { simYaw: lastSentYaw, attrs: bot.entity?.attributes })
        ...
        trace('postTickListeners') // captures pathfinder fullStop / control mutations
      }
      if (shouldUsePhysics) {
        updatePosition(now)
      }
    } finally {
      if (bot.sendClientTickEnd) bot._client.write('tick_end', {})
    }
  }

  function updateRotation () {
    // Pathfinder chooses a new heading every tick. Send and simulate that
    // heading immediately so forward movement does not orbit each path node.
  ...
  function sendPacketPosition (position, onGround) {
    ...
    trace('sendPos', { pkt: { x: lastSent.x, y: lastSent.y, z: lastSent.z, onGround } })
  ...
  function sendPacketLook (yaw, pitch, onGround) {
    ...
    trace('sendLook', { pkt: { yaw, pitch, onGround } })
    bot.emit('move', oldPos)
  }
  ...
  function sendPacketPositionAndLook (position, yaw, pitch, onGround) {
    ...
    trace('sendPosLook', { pkt: { x: lastSent.x, y: lastSent.y, z: lastSent.z, yaw, pitch, onGround } })
    bot.emit('move', oldPos)
  }
  ...
  bot.look = async (yaw, pitch, force) => {
    ...
    // Unwrap the yaw delta to its shortest equivalent (-PI, PI] before quantizing.
    // Callers such as pathfinder pass a target yaw from atan2 in (-PI, PI]; when the
    // current yaw sits on the opposite side of that branch cut, a raw subtraction is
    // ~2*PI and would rotate the entity a whole turn in one tick. On the wire this is a
    // ~360 degree snap, which trips Grim's AimModulo360. Moving by the shortest delta
    // keeps bot.entity.yaw continuous (never % 360) and the per-tick wire delta small.
    const yawChange = Math.round(deltaYaw(yaw, bot.entity.yaw) / sensitivity) * sensitivity
  ...
  bot._client.on('position', (packet) => {
    ...
    const teleportUsesOwnPacket = bot.supportFeature('teleportUsesOwnPacket')
    if (teleportUsesOwnPacket) {
    ...
    trace('forcedMove', { server: { x: pos.x, y: pos.y, z: pos.z } })
    bot.emit('forcedMove')
  })
