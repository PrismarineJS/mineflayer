...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  bot.on('move', () => {
    if (!lookingTask.done &&
      Math.abs(deltaYaw(bot.entity.yaw, lookingTask.yaw)) < 0.001 &&
      Math.abs(bot.entity.pitch - lookingTask.pitch) < 0.001) {
      lookingTask.finish()
    }
  })
  ...
  bot.look = async (yaw, pitch, force) => {
    if (!lookingTask.done) {
      console.warn('Mineflayer detected a newer look call before the previous one synced. This behavior is deprecated and may change in a future release.')
      lookingTask.finish() // finish the previous one
    }
    lookingTask = createTask()
    lookingTask.yaw = yaw
    lookingTask.pitch = pitch
