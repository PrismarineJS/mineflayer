...
function inject(bot) {
  ...
  const jumpQueued = false;
  const lastSentYaw = null;
  const positionUpdateTimer = null;
  const doPhysicsTimer = null;
  const lastPositionSentTime = null;
  const lastPhysicsFrameTime = null;
  const lastFlyingUpdate = 0;
  ...
  function nextFrame(deltaSeconds) {
    ...
    if(oldGroundSpeedSquared < EPSILON) {
      ...
      const groundFriction = physics.groundFriction * physics.walkingAcceleration;
  ...
  function sendPosition() {
    ...
    const deltaYaw = math.euclideanMod(sentPosition.yaw - lastSentYaw, PI_2);
    deltaYaw = deltaYaw < 0 ?
