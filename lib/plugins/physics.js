...
// 26.3+ では位置パケットは1tickに1回だけ許され、サーバー側の
// receivedPositionThisTick は client_tick_end (tick_end) を受け取るまで解除されない。
// 送らないと同じtickの2回目の位置パケットで
// "Invalid move player packet" キックになる。
function sendTickEnd (bot) {
  const proto = bot.registry && bot.registry.protocol
  const types = proto && proto.play && proto.play.toServer && proto.play.toServer.types
  if (types && types.packet_tick_end) bot._client.write('tick_end', {})
}
  ...
  bot._client.on('position', (packet) => {
    ...
    if (bot.supportFeature('teleportUsesOwnPacket')) {
      bot._client.write('teleport_confirm', {
        ...
        // 26.2+ は確認パケットに座標が含まれる。未定義だと NaN が書かれ
        // サーバーの containsInvalidValues に引っかかってキックされる。
        x: packet.x,
        y: packet.y,
        z: packet.z,
        yRot: packet.yaw,
