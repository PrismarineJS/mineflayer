...
function inject (bot, { physicsEnabled }) {
  ...
  function sendPacketPositionAndLookNoLastSent (position, yaw, pitch, onGround) {
    // doesn't set lastSent (only used when teleport confirming)
    const oldPos = new Vec3(lastSent.x, lastSent.y, lastSent.z)
    const posPacket = {}
    posPacket.x = position.x
    posPacket.y = position.y
    posPacket.z = position.z
    posPacket.yaw = yaw
    posPacket.pitch = pitch
    posPacket.onGround = onGround
    bot._client.write('position_look', posPacket)
    bot.emit('move', oldPos)
  }
  ...
  function updatePosition (now) {
    ...
    ticksPosition++ // increment by 1
    const positionUpdated = lastSent.x !== position.x || lastSent.y !== position.y || lastSent.z !== position.z || ticksPosition >= 20
    ...
    if (positionUpdated && lookUpdated) {
      ...
      ticksPosition = 0 // reset position timer
  ...
  bot._client.on('position', (packet) => {
    ...
    const packet = { // Vanilla doesn't set a lastSent for this
      x: pos.x,
      y: pos.y,
      z: pos.z,
      yaw: newYaw,
    ...
    sendPacketPositionAndLookNoLastSent(pos, newYaw, newPitch, bot.entity.onGround) // this doesn't reset position timer or adjust lastSent values
