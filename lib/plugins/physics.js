...
function inject (bot) {
  ...
  Object.defineProperty(bot,"physicEnabled", {
      set: (d) => bot.physicsEnabled = d,
      get: () => bot.physicsEnabled
  })
  ...
  bot.on('mount', () => { bot.physicsEnabled = false })
  bot.on('respawn', () => { bot.physicsEnabled = false })
  bot.on('end', cleanup)
}
