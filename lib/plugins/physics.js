...
function inject(bot) {
  ...
  function nextFrame(deltaSeconds) {
    ...
    // jumping
    if ((controlState.jump || jumpWasPressed) && bot.entity.onGround) {
    ...
    //}
    vel.y =
      vel.y < -physics.terminalVelocity ?
    ...
    //  if (collisionInRange(boundingBaxMin, boundingBoxMax)) {
  ...
  function sendPositionAndLook(entity) {
    ...
    var packet = {
      ...
      y: entity.position.y,
      z: entity.position.z,
      stance: entity.height + entity.position.y,
  ...
  bot.client.on(0x0d, function(packet) {
    ...
    if (positionUpdateTimer == null) {
      ...
      lastSentYaw = bot.entity.yaw % PI_2;
