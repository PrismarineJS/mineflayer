//Mineflayer's Physics with water physics added
...
function inject (bot) {
  const physics = {
    ...
    liquidPush: 1.39 // according to the wiki
  }
  ...
  function nextFrame (deltaSeconds) {
    ...
    } else if (inBlock && (inBlock.type === 9 || inBlock.type === 11)) {
      vel.add(liquidPush(pos))
  ...
  function liquidPush(pos){
    let flow = liquidFlow(pos);
    let push = flow.clone().scaled(physics.liquidPush);
    push.y = push.y/physics.gravity;
    console.log(flow);
    return push;
  }

  function liquidFlow(pos) {
    let flow = new Vec3(0,0,0);
    let base = bot.blockAt(pos).metadata;
    let id = bot.blockAt(pos).type;
    if(base > 7) {
      base = 0;
      flow.set(0,-1,0);
    }
    ...
    let Blocks = {};
    Blocks['x+'] = bot.blockAt(pos.offset(1,0,0));
    Blocks['x-'] = bot.blockAt(pos.offset(-1,0,0));
    Blocks['z+'] = bot.blockAt(pos.offset(0,0,1));
    Blocks['z-'] = bot.blockAt(pos.offset(0,0,-1));
    let waterBlocks = {}
    for(let i = 0; i < Object.keys(Blocks).length; i++){
      key = Object.keys(Blocks)[i];
      if(Blocks[key].type == id) waterBlocks[key] = depth(Blocks[key].metadata)
    }
    console.log(waterBlocks);
    if(waterBlocks['x+'] !== null && waterBlocks['x+'] > base) flow = flow.offset(1,0,0);
    if(waterBlocks['x-'] !== null && waterBlocks['x-'] > base) flow = flow.offset(-1,0,0);
    if(waterBlocks['z+'] !== null && waterBlocks['z+'] > base) flow = flow.offset(0,0,1);
    if(waterBlocks['z-'] !== null && waterBlocks['z-'] > base) flow = flow.offset(0,0,-1);
    if(waterBlocks['x+'] !== null && waterBlocks['x+'] < base && flow.x == 0) flow = flow.offset(-1,0,0)
    if(waterBlocks['x-'] !== null && waterBlocks['x-'] < base && flow.x == 0) flow = flow.offset(1,0,0);
    if(waterBlocks['z+'] !== null && waterBlocks['z+'] < base && flow.z == 0) flow = flow.offset(0,0,-1)
    if(waterBlocks['z-'] !== null && waterBlocks['z-'] < base && flow.z == 0) flow = flow.offset(0,0,1);
    return flow;
  }
