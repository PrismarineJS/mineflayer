...
const { BotcraftPhysics, EPhysicsCtx, PhysicsWorldSettings, initSetup, PlayerState, convertPlayerState, applyToPlayerState } = require('@nxg-org/mineflayer-physics-util')
...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  // compatibility with old mineflayer-pathfinder.
  // const physics1 = new EntityPhysics(bot.registry);
  ...
  settings.yawSpeed = 0.3
  settings.pitchSpeed = 0.3
  settings.overrides = {} // additional settings/attempts to modify the entity state can be used here. not sure how to make this elegant yet.
  ...
  function tickPhysics (now) {
    ...
    // Initialize if null (e.g., right after spawn before first packet)
    if (lastSentYaw === null) lastSentYaw = bot.entity.yaw
    if (lastSentPitch === null) lastSentPitch = bot.entity.pitch

    if (shouldUsePhysics) {
      // 1. Advance the interpolated yaw/pitch first
    ...
    if (bot.registry.isNewerOrEqualTo('1.21.3')) {
      bot._client.write('tick_end', { }) // 1.21.3+
    }
  }
  ...
  function updatePosition (now) {
    ...
    // Interpolation logic has been moved to tickPhysics()
    ...
    const positionUpdated =
      lengthSquared( // Client accurate implementation of movement check
        position.x - lastSent.x,
        position.y - lastSent.y,
        position.z - lastSent.z
      ) > Math.pow(2.0E-4, 2) ||
  ...
  obj.simulatePlayer = function (oldState, world) {
    ...
    /**
     * @type {EPhysicsCtx<import("@nxg-org/mineflayer-physics-util").EntityState>}>}
     */
    const ectx = EPhysicsCtx.FROM_ENTITY_STATE(physics, state, bot.registry.entitiesByName["player"], settings)
    // ectx.state.prevControl = ectx.state.control
    // ectx.state.prevHeading = { forward: 0, strafe: 0}
    // ectx.state.effects = bot.entity.effects;
