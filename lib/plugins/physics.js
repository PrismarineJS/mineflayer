...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  function answerTeleport (teleportId, pos, yaw, pitch) {
    if (bot.supportFeature('teleportUsesOwnPacket')) {
      // 26.3+ widened accept_teleportation to echo back the resolved
      // position/rotation: with only the id the remaining fields are
      // undefined (NaN on the wire) and the server kicks the bot with
      // "Invalid move player packet received". Older protocols ignore
      // the extra fields.
