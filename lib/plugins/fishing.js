...
const { createDoneTask, createCompatibleTask, tasks } = require('../promise_utils')
...
function inject (bot) {
  ...
  async function fish () {
    ...
    // await waitForTaskCompatibility(tasks.fishing_fish)
    fishingTask = createCompatibleTask(tasks.fishing_fish)
