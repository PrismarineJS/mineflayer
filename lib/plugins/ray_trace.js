...
module.exports = (bot) => {
  ...
  bot.playerAtCursor = (maxDistance = 3.5) => {
    let players = Object.keys(bot.players).map((key) => bot.players[key])
      .filter((player) => {return player.username !== bot.username && player.entity &&
    ...
    let dir = new Vec3(-Math.sin(bot.entity.yaw) * Math.cos(bot.entity.pitch), Math.sin(bot.entity.pitch), -Math.cos(bot.entity.yaw) * Math.cos(bot.entity.pitch))
    let iterator = new RaycastIterator(bot.entity.position.offset(0, bot.entity.height, 0), dir.normalize(), maxDistance);

    let pos = iterator.next();
    while (pos) {
      for (let i = 0; i < players.length; i++) {
        let player = players[i].entity;
        let w = player.width / 2;

        let shapes = [[-w, 0, -w, w, player.height + 0.18, w]];
