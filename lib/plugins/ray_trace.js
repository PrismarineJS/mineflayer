...
module.exports = (bot) => {
  ...
  bot.entityAtCursor = (maxDistance = 3.5) => {
    const entities = Object.values(bot.entities)
      .filter(entity => {
        if (entity.type === 'mob') {
          return entity.position.distanceSquared(bot.entity.position) <= maxDistance ** 2
        } else if (entity.type === 'player') {
          return entity.username !== bot.entity.username && entity.position.distanceSquared(bot.entity.position) <= maxDistance ** 2
        } else {
          return false
        }
      })
