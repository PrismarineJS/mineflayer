...
function inject (bot, { version }) {
  ...
  async function placeBlockWithOptions(referenceBlock, faceVector, options) {
    // 0 is placing with mainhand whilst 1 is off-hand
    let handToPlaceWith = 0
    if (options.handToUse === "off-hand") {
      // slot #45 is the offhand slot
      if (!bot.inventory.slots[45]) {
        throw new Error('must be holding an item in the off-hand to place a block')
      } else handToPlaceWith = 1
    } else if (!bot.heldItem) throw new Error('must be holding an item to place a block')
