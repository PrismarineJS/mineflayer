 ...
 * @param { { version: string, storageBuilder: any} } param1
 */
function inject (bot, { version, storageBuilder }) {
  ...
  // Emit open_sign event
  bot._client.on('open_sign_entity', (data) => {
    const { location: position } = data // Checked for 1.8 - 1.18.2
    bot.emit('signOpen', { position })
  })
  ...
  /**
   * Places a sign and writes to it
   * @throws {Error} Throws error when no sign is equipped or the position to place the sign is is already occupied
   * @param {import('vec3').Vec3} pos
   * @param {string} text
   * @param { { sneak?: boolean, writeDelay?: number, forceLook?: boolean } } options
   * @returns {Promise<void>}
   */
  bot.placeSign = async (pos, text, options = {}) => {
    if (!(pos instanceof Vec3)) throw new Error('Invalid argument for pos. Must be a instance of Vec3 got ' + typeof pos)
    if (!bot.heldItem || (bot.heldItem && !bot.heldItem.name.includes('sign'))) throw new Error('Need a sign equipped to place sign')
    const signBlock = bot.blockAt(pos)
    if (!signBlock || !signBlock?.position || (signBlock && !signBlock.name.includes('air'))) throw new Error('Invalid sign position')
    const forceLook = options.forceLook ?? true
    const groundPlacable = bot.blockAt(signBlock.position.offset(0, -1, 0))?.boundingBox === 'block'
    let wallPlacement = null
    const directions = [new Vec3(1, 0, 0), new Vec3(0, 0, 1), new Vec3(-1, 0, 0), new Vec3(0, 0, -1)]
    for (const dir of directions) {
      const tmp = bot.blockAt(pos.minus(dir))
      if (tmp?.boundingBox === 'block') {
        wallPlacement = dir
        break
      }
    }

    if (!groundPlacable && wallPlacement === null) throw new Error('Cannot place block in position ' + pos.toString())
    ...
    const signGuiOpen = onceWithCleanup(bot, 'signOpen', {
      ...
      checkCondition: (data) => {
        ...
        return new Vec3(x, y, z).distanceTo(pos) === 0
      }
    })

    // Start both tasks at once: Block placement and listening for the sign gui opening.
    // The gui open event already fired when we await the placeBlock. But both have to be complete to continue.
    const placeVector = groundPlacable ? new Vec3(0, 1, 0) : wallPlacement
    const placeBlock = bot.blockAt(pos.minus(placeVector))
    if (options.sneak) bot.setControlState('sneak', true)
    const placeBlockProm = bot._placeBlockWithOptions(placeBlock, placeVector, { swingArm: 'right', forceLook })
    await Promise.allSettled([placeBlockProm, signGuiOpen])
    if (options.writeDelay) await new Promise((resolve) => setTimeout(resolve, options.writeDelay)) // Workaround for anti cheat "AutoSign" check
    if (options.sneak) bot.setControlState('sneak', false)
    bot.updateSign(bot.blockAt(pos), text)
  }
