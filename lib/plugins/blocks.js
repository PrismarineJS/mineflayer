...
function inject (bot, { version, storageBuilder }) {
  ...
  function createNewChunk () {
    const chunkInitializationData = {}
    const dimensionData = bot.game.dimensionData

    if (dimensionData) {
      chunkInitializationData.minWorldHeight = dimensionData.min_y
      chunkInitializationData.maxWorldHeight = dimensionData.min_y + dimensionData.height
    }

    return new Chunk(chunkInitializationData)
  }
  ...
  bot._client.on('update_light', (packet) => {
    ...
    // Before 1.13, light packet contained combined skylight and blocklight data
    // Now they are stored separately in their exclusive fields, and their type are arrays
    if (packet.data) {
      ...
      // Use new method for handling 1.13+ lighting data
      const flatSkyLightMask = flattenMaskArray(packet.skyLightMask)
      const flatBlockLightMask = flattenMaskArray(packet.blockLightMask)

      column.loadLightFromSectionData(packet.skyLight, packet.blockLight, flatSkyLightMask, flatBlockLightMask)
    }
  })
  ...
  if (bot.supportFeature('usesNewEntityDestroyFormat')) {
    bot._client.on('destory_entity', (packet) => {
      const painting = paintingsById[packet.entityId]
      if (painting) deletePainting(painting)
    })
  } else {
