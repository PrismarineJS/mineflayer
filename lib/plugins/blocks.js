...
function inject(bot) {
  ...
  function blockAt(absolutePoint) {
    ...
    block.meta = nybble(chunk.buffer.readUInt8(offset), blockIndex % 2 === 1);

    offset = BLOCKS_PER_CHUNK * 1.5 + blockIndex * 0.5;
    block.light = nybble(chunk.buffer.readUInt8(offset), blockIndex % 2 === 1);

    var addArrayBegin;
    if (chunk.skyLightSent) {
      offset = BLOCKS_PER_CHUNK * 2 + blockIndex * 0.5;
      block.skyLight = nybble(chunk.buffer.readUInt8(offset), blockIndex % 2 === 1);
    ...
    if (chunk.addBitMap & (1 << blockPoint.y)) {
      ...
      block.add = nybble(chunk.buffer.readUInt8(offset), blockIndex % 2 === 1);
    }
