...
function inject (bot, { version }) {
  ...
  function delColumn (chunkX, chunkY) {
    ...
    // bot.world.unloadColumn(chunkX, chunkY) // TODO make this work
  ...
  function addColumn (args) {
    ...
    let column = bot.world.sync.getColumn(args.x, args.z)
    if (!column) {
      column = new Chunk()
      bot.world.sync.setColumn(args.x, args.z, column)
    }
  ...
  bot.findBlocks = (options) => {
    ...
    let next = start
    while (next) {
      const column = bot.world.sync.getColumn(next.x, next.y)
  ...
  function blockAt (absolutePoint, extraInfos = true) {
    ...
    const column = bot.world.sync.getColumn(loc.chunkCorner.x, loc.chunkCorner.z)
  ...
  bot._client.on('update_light', (packet) => {
    let column = bot.world.sync.getColumn(packet.chunkX, packet.chunkZ)
    if (!column) {
      column = new Chunk()
      bot.world.sync.setColumn(packet.chunkX, packet.chunkZ, column)
    }
