...
function inject (bot, { version, storageBuilder, hideErrors }) {
  ...
  bot._client.on('chunk_batch_finished', (packet) => {
    ...
    const ClampedMilliPerChunk = Math.min(Math.max(MilliPerChunk, weightedAverage / 3.0), weightedAverage * 3.0) // Prevents the MilliPerChunk from being hugely different then the average.
