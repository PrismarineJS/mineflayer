...
function inject (bot, { version, storageBuilder, hideErrors }) {
  ...
  let signEditor = null
  // Since 1.21.5 sign lines are stored as NBT text components instead of JSON strings
  const signTextIsNbtComponent = bot.supportFeature('signTextUsesNbtComponents')
  ...
  function blockInteractionRange () {
    ...
    for (const key in attributes) {
      if (String(key).endsWith('block_interaction_range')) return attributes[key].value
    }
    return 4.5
  }
