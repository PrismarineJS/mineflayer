...
function inject (bot, { version, storageBuilder }) {
  ...
  function canSeeBlock (block) {
    ...
    const blockAtCursor = bot.world.raycast(headPos, dir.normalize(), 5)
  ...
  function startListenerProxy () {
    if (listenerState) {
      // remove the forwarders if they already exist to prevent performance issues
      bot.removeListener('removeListener', listenerState)
      bot.removeListener('newListener', listenerState)
    }
    ...
    // used to remove these specific listeners in case other code requires this listener
    listenerState = (event, listener) => {
      if (blockUpdateRegex.test(event)) {
        bot.world.on(event, listener)
      }
    }
    // custom forwarder for custom events
    bot.on('newListener', listenerState)
    bot.on('removeListener', listenerState)
  }
