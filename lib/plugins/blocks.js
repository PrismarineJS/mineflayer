...
function inject (bot, { version, storageBuilder }) {
  ...
  bot._client.on('multi_block_change', (packet) => {
    ...
    for (let i = 0; i < packet.records.length; ++i) {
      ...
      const blockZ = record.horizontalPos !== undefined ? (record.horizontalPos & 0x0f) : record & 0x0f
      const blockX = record.horizontalPos !== undefined ? (record.horizontalPos >> 4) & 0x0f : (record >> 4) & 0x0f
      const blockY = record.y !== undefined ? record.y : (record >> 8)

      const pt = new Vec3((packet.chunkX !== undefined ? packet.chunkX : packet.chunkCoordinates.x) * 16 + blockX, (packet.chunkCoordinates !== undefined ? packet.chunkCoordinates.y : 0) * 16 + blockY, (packet.chunkZ !== undefined ? packet.chunkZ : packet.chunkCoordinates.z) * 16 + blockZ)
      updateBlockState(pt, (record.blockId !== undefined ? record.blockId : record >> 12))
    }
  })
