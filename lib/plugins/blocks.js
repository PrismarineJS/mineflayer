...
function inject (bot, { version, storageBuilder }) {
   ...
   * @param {import('prismarine-block').Block} refBlock
   * @param {import('vec3').Vec3} direction
   * @param {string|Array<string>} text
   * @param { { sneak?: boolean, writeDelay?: number, forceLook?: boolean } } options
   * @returns {Promise<void>}
   */
  bot.placeSign = async (refBlock, direction, text, options = {}) => {
    ...
    const signBlock = bot.blockAt(refBlock.position.plus(direction))
    if (refBlock && refBlock.name.includes('air')) throw new Error('Invalid sign position')
    if (!signBlock || (signBlock && !signBlock.name.includes('air'))) throw new Error('Invalid sign position')
    ...
    const signGuiOpen = onceWithCleanup(bot, 'signOpen', {
      ...
      checkCondition: (data) => {
        ...
        return new Vec3(x, y, z).distanceTo(signBlock.position) === 0
      }
    })
    ...
    const placeBlockProm = bot._placeBlockWithOptions(refBlock, direction, { swingArm: 'right', forceLook })
    ...
    bot.updateSign(bot.blockAt(signBlock.position), text) // Upddate the sign block again
  }
