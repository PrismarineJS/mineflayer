...
function inject (bot, { version, storageBuilder, hideErrors }) {
  ...
  bot.findChests = (options) => {
    ...
    options.matching = (block) => {
      if (block.type !== chestBlockId && block.type !== trappedChestBlockId) return false

      // Only match single chests and the right side of double chests
      const chestType = bot._getChestType(block)
      if (chestType === 'single' || chestType === 'right') return true
    }
