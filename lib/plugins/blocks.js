...
function inject (bot, { version, storageBuilder }) {
  const SIGN_BLOCKENTITY_TYPE = 7 // TODO: add in id field for rest of block entities
  ...
  function addExtraBlockEntityData (blockType, tag) {
    const extra = { raw: tag, type: blockType }
    if (blockType === 'minecraft:sign' || blockType === 'Sign') {
      const prepareJson = (i) => {
        const data = tag[`Text${i}`]
      ...
      extra.Text1 = new ChatMessage(prepareJson(1))
      extra.Text2 = new ChatMessage(prepareJson(2))
      extra.Text3 = new ChatMessage(prepareJson(3))
      extra.Text4 = new ChatMessage(prepareJson(4))
    }
    return extra
  }
  ...
  function blockAt (absolutePoint, extraInfos = true) {
    ...
    if (extraInfos) {
      ...
      if (block.entity) {
        const id = block.entity.value.id.value
        Object.assign(block.blockEntity, addExtraBlockEntityData(id, block.entity))
        if (id === 'minecraft:sign' || id === 'Sign') {
          block.signText = [block.blockEntity.Text1, block.block.Text2, block.block.Text3, block.block.Text4]
        }
      }
    }
    ...
    if (typeof packet.blockEntities !== 'undefined') {
      ...
      for (const blockEntity of packet.blockEntities) {
        if (blockEntity.x !== undefined) { // 1.17+
          if (!blockEntity.type === SIGN_BLOCKENTITY_TYPE) {
            blockEntity.nbtData.value.id.value = 'minecraft:sign'
          }
          ...
          added.push([blockEntity.value.x.value, blockEntity.value.y.value, blockEntity.value.z.value])
        }
      }
    }
  })
  ...
  bot._client.on('update_sign', (packet) => {
    ...
    if (!blockAt.entity) {
      blockAt.entity = nbt.comp({
        id: nbt.string('minecraft:sign')
      })
    }

    blockAt.entity.value.Text1 = nbt.string(prepareString(1))
    blockAt.entity.value.Text2 = nbt.string(prepareString(2))
    blockAt.entity.value.Text3 = nbt.string(prepareString(3))
    blockAt.entity.value.Text4 = nbt.string(prepareString(4))
  })

  bot._client.on('tile_entity_data', (packet) => {
    if (packet.type === SIGN_BLOCKENTITY_TYPE) { // Sign, todo: make not constant
      packet.nbtData.value.id.value = 'minecraft:sign'
    }
  ...
  Object.defineProperty(bot, '_blockEntities', {
    get () {
      console.warn('_blockEntities is deprecated, use bot.world instead')
      return {}
    },
    enumerable: true,
    configurable: true
