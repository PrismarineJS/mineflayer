...
function inject (bot, { version, storageBuilder }) {
  ...
  bot.findBlocks = (options) => {
    assert(
      typeof options.matching === 'function' ||
      typeof options.matching === 'number' ||
      (Array.isArray(options.matching) && typeof options.matching[0] === 'number') ||
      options.matching === undefined,
      Error('options.matching must be a function, number[], or number')
    )
    assert(
      options.point instanceof Vec3 ||
      options === undefined,
      Error('options.point must be a Vec3 point')
    )
    assert(
      (typeof options.maxDistance === 'number' && options.maxDistance > 0) ||
      options.maxDistance === undefined,
      Error('options.maxDistance must be a number that is >0')
    )
    assert(
      (typeof options.count === 'number' && options.count > 0) ||
      options.count === undefined,
      Error('options.count must be a number that is >0')
    )
    assert(
      typeof options.useExtraInfo === 'function' ||
      typeof options.useExtraInfo === 'boolean',
      Error('options.useExtraInfo must be a boolean')
    )
