...
function inject(bot) {
  ...
  function blockAt(absolutePoint) {
    ...
    var lowNybble = blockIndex % 2 === 1;

    offset = BLOCKS_PER_CHUNK + blockIndex * 0.5;
    block.meta = nybble(chunk.buffer.readUInt8(offset), lowNybble);

    offset = BLOCKS_PER_CHUNK * 1.5 + blockIndex * 0.5;
    block.light = nybble(chunk.buffer.readUInt8(offset), lowNybble);

    var addArrayBegin;
    if (chunk.skyLightSent) {
      offset = BLOCKS_PER_CHUNK * 2 + blockIndex * 0.5;
      block.skyLight = nybble(chunk.buffer.readUInt8(offset), lowNybble);
    ...
    if (chunk.addBitMap & (1 << blockPoint.y)) {
      ...
      block.add = nybble(chunk.buffer.readUInt8(offset), lowNybble);
    }
