...
function inject(bot, { version, storageBuilder, hideErrors }) {
  ...
  function addColumn(args) {
    const viewDist = bot.settings.viewDistance ?? 8
    const botChunkX = Math.floor(bot.entity.position.x / 16)
    const botChunkZ = Math.floor(bot.entity.position.z / 16)
    const dx = args.x - botChunkX
    const dz = args.z - botChunkZ
  ...
  function getMatchingFunction(matching) {
    if (typeof matching !== 'function') {
      if (!Array.isArray(matching)) matching = [matching]
  ...
  function isBlockInSection(section, matcher) {
    ...
    if (section.palette) {
      for (const stateId of section.palette) {
        if (matcher(Block.fromStateId(stateId, 0))) return true
      }
      return false
    }
    return true
  ...
  function getFullMatchingFunction(matcher, useExtraInfo) {
    if (typeof useExtraInfo === 'boolean') return fullSearchMatcher
  ...
  bot.findBlocks = options => {
    ...
    while (next) {
      ...
      if (startedLayer !== it.apothem && blocks.length >= count) break
  ...
  function blockAt(absolutePoint, extraInfos = true) {
    ...
    if (extraInfos) block.painting = paintingsByPos[block.position]
    return block
  }
  ...
  bot._client.on('map_chunk', packet => {
    ...
    if (typeof packet.blockEntities !== 'undefined') {
      ...
      for (const blockEntity of packet.blockEntities) {
        if (blockEntity.x !== undefined) column.setBlockEntity(blockEntity, blockEntity.nbtData)
        else {
  ...
  bot._client.on('map_chunk_bulk', packet => {
    ...
    for (i = 0; i < packet.meta.length; ++i) {
      meta = packet.meta[i]
      size = (8192 + (packet.skyLightSent ? 2048 : 0)) * onesInShort(meta.bitMap) + 2048 * onesInShort(meta.bitMap) + 256
      addColumn({
        x: meta.x,
        z: meta.z,
        bitMap: meta.bitMap,
  ...
  bot._client.on('multi_block_change', packet => {
    for (let i = 0; i < packet.records.length; ++i) {
      ...
      if (bot.supportFeature('usesMultiblockSingleLong')) updateBlockState(pt, record >> 12)
      else updateBlockState(pt, record.blockId)
    }
  })
  ...
  function dimensionToFolderName(dimension) {
    if (bot.supportFeature('dimensionIsAnInt')) return dimensionNames[dimension]
    else if (bot.supportFeature('dimensionIsAString') || bot.supportFeature('dimensionIsAWorld')) return worldName
  }
  bot._getDimensionName = () => worldName

  async function switchWorld() {
    if (bot.world) {
      if (storageBuilder) await bot.world.async.waitSaving()
  ...
  bot._client.on('login', packet => {
    ...
    } else if (bot.supportFeature('spawnRespawnWorldDataField')) {
  ...
  bot._client.on('respawn', packet => {
    if (bot.supportFeature('dimensionIsAnInt')) {
      ...
      if (worldName === packet.worldState.name && packet.copyMetadata === true) return
      dimension = packet.worldState.dimension
      worldName = packet.worldState.name
    } else {
      if (dimension === packet.dimension) return
      if (worldName === packet.worldName && packet.copyMetadata === true) return
      dimension = packet.dimension
  ...
  function startListenerProxy() {
    ...
    listener = (event, listener) => {
      if (blockUpdateRegex.test(event)) bot.world.on(event, listener)
    }
    listenerRemove = (event, listener) => {
      if (blockUpdateRegex.test(event)) bot.world.off(event, listener)
    }
