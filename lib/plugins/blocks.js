...
const { getAttributeValue } = require('../attributeValue')
...
// Names the vanilla client renders for its own keybind components with default key bindings
// (used when a sign holds a {"keybind": ...} component, see the sign editor emulation below).
const vanillaKeybindNames = {
...
function inject (bot, { version, storageBuilder, hideErrors }) {
  ...
  bot._client.on('unload_chunk', (packet) => {
    ...
    if (signEditor && (signEditor.position.x >> 4) === packet.chunkX && (signEditor.position.z >> 4) === packet.chunkZ) {
      invalidateSignEditor()
    }
  })
  ...
  function updateBlockState (point, stateId) {
    ...
    if (oldBlock.type !== newBlock.type) {
      ...
      // a different block replaces the block entity too, like the vanilla client does
      if (oldBlock.entity) {
        const column = bot.world.getColumn(point.x >> 4, point.z >> 4)
        if (column && typeof column.removeBlockEntity === 'function') {
          column.removeBlockEntity(new Vec3(point.x & 0xf, point.y, point.z & 0xf))
        }
      }
      if (signEditor && signEditor.position.equals(pos)) invalidateSignEditor()
    }
  }
  ...
  // --- Sign editor emulation ---
  // The vanilla client opens its sign edit screen on open_sign_entity and always answers with
  // update_sign when that screen closes: when the player is done (bot.updateSign), when the sign
  // block is removed or the chunk unloads, when the sign is out of reach, when another screen
  // opens (a window, a new sign editor) or when the world changes. The lines it sends back are
  // the sign's current text rendered to plain strings, so translatable and keybind components
  // are resolved with the client's language and key bindings. Anti-cheat plugins flash a
  // phantom sign full of translation probes and treat a client that never answers as an
  // anti-detection mod, so the same flow is mirrored here.
  let signEditor = null
  // Since 1.21.5 sign lines are stored as NBT text components instead of JSON strings. Fall back to a version check when
  // the signTextUsesNbtComponents feature is not yet in the installed minecraft-data, so 1.21.5+ NBT-string lines that
  // happen to look like JSON are not JSON.parsed and mangled.
  const signTextIsNbtComponent = bot.supportFeature('signTextUsesNbtComponents') || bot.registry.version['>=']('1.21.5')

  function resolveVanillaKeybinds (json) {
    if (Array.isArray(json)) return json.map(resolveVanillaKeybinds)
    if (!json || typeof json !== 'object') return json
    const out = { ...json }
    if (typeof out.keybind === 'string' && vanillaKeybindNames[out.keybind] !== undefined) {
      out.text = vanillaKeybindNames[out.keybind]
      delete out.keybind
    }
    if (out.extra) out.extra = resolveVanillaKeybinds(out.extra)
    if (out.with) out.with = resolveVanillaKeybinds(out.with)
    return out
  }

  // One sign line as stored in the block entity (simplified NBT) -> the plain string a vanilla client shows
  function signLineToPlainText (message) {
    if (message === undefined || message === null) return ''
    let json = message
    if (typeof message === 'string' && !signTextIsNbtComponent) {
      if (message === '' || message === 'null') return ''
      try { json = JSON.parse(message) } catch (e) { json = message }
    }
    try {
      return new ChatMessage(resolveVanillaKeybinds(json)).toString()
    } catch (e) {
      return typeof message === 'string' ? message : ''
    }
  }

  function signEditorLines (block, isFrontText) {
    const lines = ['', '', '', '']
    const data = block && block.blockEntity
    if (!data) return lines
    let messages
    if (bot.supportFeature('multiSidedSigns')) {
      const side = data[isFrontText ? 'front_text' : 'back_text']
      messages = side && Array.isArray(side.messages) ? side.messages : []
    } else {
      messages = [data.Text1, data.Text2, data.Text3, data.Text4]
    }
    return lines.map((_, i) => signLineToPlainText(messages[i]).slice(0, 384))
  }

  function signPacketText (lines) {
    const stringified = bot.supportFeature('sendStringifiedSignText')
    const text = {}
    for (let i = 0; i < 4; i++) {
      const line = lines[i] ?? ''
      text[`text${i + 1}`] = stringified ? JSON.stringify(line) : line
    }
    return text
  }

  // the vanilla screen notices a removed sign on its next tick, so answer one tick later
  function invalidateSignEditor () {
    if (!signEditor || signEditor.closing) return
    const editor = signEditor
    editor.closing = setTimeout(() => { if (signEditor === editor) closeSignEditor() }, 50)
  }
  ...
  function closeSignEditor () {
    ...
    const editor = signEditor
    signEditor = null
    if (editor.closing) clearTimeout(editor.closing)
    // A vanilla client only answers with update_sign from the play state. Entering configuration or losing the
    // connection discards the editor instead of writing: there is no update_sign packet in those states and the
    // server's configuration parser reports a partial read for it.
    if (bot._client.state !== 'play') return
    bot._client.write('update_sign', {
      location: editor.position,
      isFrontText: editor.isFrontText,
      ...signPacketText(editor.lines)
    })
  }

  // Drop the editor without answering - for configuration/end and world replacement, where a submit is invalid, and so a
  // delayed close (invalidateSignEditor) that crosses that boundary cannot fire for a superseded session.
  function discardSignEditor () {
    if (!signEditor) return
    if (signEditor.closing) clearTimeout(signEditor.closing)
    signEditor = null
  }
  bot._client.on('state', state => { if (state !== 'play') discardSignEditor() })
  bot._client.on('end', discardSignEditor)

  bot._client.on('open_sign_entity', (packet) => {
    const pos = new Vec3(packet.location.x, packet.location.y, packet.location.z)
    const isFrontText = packet.isFrontText ?? true
    closeSignEditor() // a new screen replaces the previous one, which sends its lines
    const block = bot.blockAt(pos)
    signEditor = { position: pos, isFrontText, lines: signEditorLines(block, isFrontText) }
    bot.emit('signOpen', block, isFrontText)
  })

  // the player's block interaction range attribute (1.20.5+), default 4.5 blocks
  function blockInteractionRange () {
    const attributes = bot.entity.attributes
    if (!attributes) return 4.5
    for (const key in attributes) {
      // The effective range (base plus attribute modifiers), not the transmitted base value, matches the vanilla screen.
      if (String(key).endsWith('block_interaction_range')) return getAttributeValue(attributes[key])
    }
    return 4.5
  }

  // the vanilla screen closes itself once the sign is out of the player's block interaction range
  bot.on('physicsTick', () => {
    if (!signEditor || !bot.entity || !bot.entity.position) return
    const eye = bot.entity.position.offset(0, bot.entity.eyeHeight ?? 1.62, 0)
    const p = signEditor.position
    const dx = Math.max(p.x - eye.x, 0, eye.x - (p.x + 1))
    const dy = Math.max(p.y - eye.y, 0, eye.y - (p.y + 1))
    const dz = Math.max(p.z - eye.z, 0, eye.z - (p.z + 1))
    const reach = blockInteractionRange() + 4
    if (dx * dx + dy * dy + dz * dz >= reach * reach) closeSignEditor()
  })

  for (const packetName of ['open_window', 'open_horse_window', 'open_book', 'close_window', 'respawn']) {
    bot._client.on(packetName, () => closeSignEditor())
  }
  bot.on('death', () => closeSignEditor())
  ...
  bot.updateSign = (block, text, back = false) => {
    ...
    if (signEditor && signEditor.position.equals(block.position)) {
      if (signEditor.closing) clearTimeout(signEditor.closing)
      signEditor = null
    }
    bot._client.write('update_sign', {
