...
function inject (bot, { version, storageBuilder, hideErrors }) {
          ...
          for (cursor.x = begin.x; cursor.x < end.x; cursor.x++) {
            for (cursor.y = begin.y; cursor.y < end.y; cursor.y++) {
              for (cursor.z = begin.z; cursor.z < end.z; cursor.z++) {
                if (cursor.distanceTo(point) <= maxDistance && fullMatcher(cursor)) blocks.push(cursor.clone())
              }
            }
          }
        }
  ...
  function getVisibleBlockFaces (block) {
    const visibleFaces = []
    const observerToBlock = block.position.offset(0.5, 0.5, 0.5).minus(bot.entity.position.offset(0, bot.entity.eyeHeight, 0))
    for (const blockFace of blockFaceToVec) {
      const dotProduct = blockFace.dot(observerToBlock)
    ...
    // Sort by ascending dotProduct (most visible faces first)
    visibleFaces.sort((a, b) => a.dotProduct - b.dotProduct)
  ...
  function canSeeBlock (block) {
    ...
    if (blockAtCursor && blockAtCursor.position.equals(block.position)) { return true }

    //If the direct raycast did not yield a result, 
    //try raycasting to the visible faces of the block
    let visibleFaces = getVisibleBlockFaces(block)
    // Remove the most visible face (already checked above)
    visibleFaces.shift()
    for (let visibleFace of visibleFaces) {
      // Calculate the position at the center of the face
      const facePosition = block.position.offset(0.5, 0.5, 0.5).offset(visibleFace.face.x / 2, visibleFace.face.y / 2, visibleFace.face.z / 2);
      const rangeToFace = headPos.distanceTo(facePosition) + 0.01
      const dirToFace = facePosition.minus(headPos)
      const blockAtCursor = bot.world.raycast(headPos, dirToFace.normalize(), rangeToFace, match)

      if (blockAtCursor && blockAtCursor.position.equals(block.position)) {
        return true;
      }
    }
    // If none of the faces are visible, return false
    return false;
  ...
  bot.getVisibleBlockFaces = getVisibleBlockFaces
