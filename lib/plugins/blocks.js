...
function inject (bot, { version }) {
  ...
  const paintingsById = {}
  const loadingPool = workerpool.pool()
  ...
  function addColumn (args) {
    ...
    loadingPool.exec((data, bitMap, skyLightSent, version) => {
      const Chunk = require('prismarine-chunk')(version)
      const chunk = new Chunk()
      chunk.load(Buffer.from(data), bitMap, skyLightSent)
      return chunk.toJson()
    }, [args.data, args.bitMap, args.skyLightSent, version]).then(chunk => {
      columns[key] = Chunk.fromJson(chunk)
