...
function inject (bot) {
  ...
  let diggingAction = actionCreateDone()
  ...
  async function dig (block, forceLook, digFace) {
    ...
    diggingAction = bot.actionCreateCompatible(actions.digging_dig)
    ...
    bot.stopDigging = () => {
      ...
      diggingAction.cancel(new Error('Digging aborted'))
    }
