...
// 26.3 の block_dig は sequence (varint) が必須。無いとサーバーが受理せず、
// クライアント側だけブロックが消えてアイテムもドロップしない。
let digSequence = 0
function nextDigSequence () {
  digSequence = (digSequence + 1) % 32767
  return digSequence
}
...
function inject (bot) {
  ...
  async function dig (block, forceLook, digFace) {
    ...
    bot._client.write('block_dig', {
      ...
      sequence: nextDigSequence() // 26.3 は sequence が必須(無いとサーバーが受理しない)
    })
