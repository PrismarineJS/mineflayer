...
function inject (bot) {
  let abortDigging = null // abort of the dig in progress, takes the abort face
  ...
  async function dig (block, forceLook, digFace) {
    ...
    // This dig's state is local and installed before anything below can complete it: an instant break
    // or a block update emits diggingCompleted synchronously, and a listener may start the next dig
    ...
    function finishDigging () {
      ...
      bot._client.write('block_dig', {
        ...
        location: block.position,
        face: targetDigFace, // always the same as the start face
        sequence: bot._nextSequence()
      })
      bot.targetDigBlock = null
      bot.targetDigFace = null
    ...
    function abort (face) {
      ...
      task.cancel(new Error('Digging aborted'))
    }
