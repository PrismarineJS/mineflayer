...
const { createDoneTask, createCompatibleTask, tasks, waitForTaskCompatibility } = require('../promise_utils')
...
function inject (bot) {
  ...
  async function dig (block, forceLook, digFace) {
    ...
    await waitForTaskCompatibility(tasks.digging_dig)
    diggingTask = createCompatibleTask(tasks.digging_dig)
