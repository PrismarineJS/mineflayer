...
function inject (bot) {
  ...
  function stopSwinging () {
    if (swingTickListener) {
      bot.removeListener('physicsTick', swingTickListener)
      swingTickListener = null
    }
  }
  ...
  async function dig (block, forceLook, digFace) {
    ...
    bot.on('physicsTick', swingTickListener)
