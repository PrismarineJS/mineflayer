...
function inject (bot) {
  ...
  bot._parseChestMetadata = (chestBlock) => {
  ...
  bot._getChestType = (chestBlock) => { // Returns 'single', 'right' or 'left'
    if (bot.supportFeature('doesntHaveChestType')) {
      const facing = bot._parseChestMetadata(chestBlock).facing
      ...
      return bot._parseChestMetadata(chestBlock).type
    }
  }
  ...
  bot._client.on('block_action', (packet) => {
    ...
    if (blockName === 'noteblock') { // Pre 1.13
      ...
      if (blockName === 'chest' || blockName === 'trapped_chest') {
        const chestType = bot._getChestType(block)
        if (chestType === 'right') {
          const index = Object.values(CHEST_FACING_MAP[bot._parseChestMetadata(block).facing]).indexOf('left')
          const cardinalBlock2 = Object.keys(CHEST_FACING_MAP[bot._parseChestMetadata(block).facing])[index]
