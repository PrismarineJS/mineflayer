...
function inject (bot) {
  ...
  bot._client.on('block_action', (packet) => {
    console.log('Received block_action packet:', packet);
    ...
    if (blockName === 'noteblock') { // Pre 1.13
      console.log('Emitting noteHeard for noteblock:', block, instruments[packet.byte1], packet.byte2);
      bot.emit('noteHeard', block, instruments[packet.byte1], packet.byte2)
    } else if (blockName === 'note_block') { // 1.13 onward
      console.log('Emitting noteHeard for note_block:', block, instruments[Math.floor(block.metadata / 50)], Math.floor((block.metadata % 50) / 2));
