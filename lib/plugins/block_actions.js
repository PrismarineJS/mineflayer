...
function inject (bot) {
  ...
  bot.parseChestMetadata = (chestBlock) => {
  ...
  bot.getChestType = (chestBlock) => { // Returns 'single', 'right' or 'left'
    if (bot.supportFeature('doesntHaveChestType')) {
      const facing = bot.parseChestMetadata(chestBlock).facing
      ...
      return bot.parseChestMetadata(chestBlock).type
    }
  }
  ...
  bot._client.on('block_action', (packet) => {
    ...
    if (blockName === 'noteblock') { // Pre 1.13
      ...
      if (blockName === 'chest' || blockName === 'trapped_chest') {
        const chestType = bot.getChestType(block)
        if (chestType === 'right') {
          const index = Object.values(CHEST_FACING_MAP[bot.parseChestMetadata(block).facing]).indexOf('left')
          const cardinalBlock2 = Object.keys(CHEST_FACING_MAP[bot.parseChestMetadata(block).facing])[index]
