...
function inject (bot, { version, hideErrors }) {
  ...
  async function waitForWindowUpdate (windowType, slot) {
    if (windowType === 'minecraft:inventory') {
      if (slot >= 1 && slot <= 4) {
        await once(bot.inventory, 'updateSlot:0')
      }
    } else if (windowType === 'minecraft:crafting') {
      if (slot >= 1 && slot <= 9) {
  ...
  async function clickWindow (slot, mouseButton, mode) {
    ...
    if (!window.transactionRequiresConfirmation(click) && bot.supportFeature('transactionPacketExists')) {
      ...
      await waitForWindowUpdate(window.type, slot)
    }
  }
