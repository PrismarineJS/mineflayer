...
function inject (bot, { version, hideErrors }) {
  ...
  async function clickWindow (slot, mouseButton, mode) {
    ...
    const update = once(window, `updateSlot:${slot}`)
    ...
    const puttingItemIntoWindow = !!bot.heldItem
    if (bot.supportFeature('stateIdUsed')) { // 1.17.1 +
      bot._client.write('window_click', {
        ...
        changedSlots: [{
          location: slot,
          item: puttingItemIntoWindow ? Item.toNotch(window.slots[slot]) : Item.toNotch(null)
        }],
        cursorItem: puttingItemIntoWindow ? Item.toNotch(null) : Item.toNotch(bot.heldItem)
      })
