...
function inject(bot) {
  ...
  function craft(itemType, options, cb) {
    if (! cb && options) {
      cb = options;
      options = {};
    }
    ...
    var recipe = getRecipe(itemType, metadata, count, craftingTable);
    if (! recipe) {
      cb(new Error("Could not find recipe matching requirements."));
      return;
    }
  }

  function getRecipe(itemType, metadata, count, craftingTable) {
    ...
    for (i = 0; i < possibleRecipes.length; ++i) {
      ...
      if (recipe.metadata === metadata &&
          requirementsMetForRecipe(recipe, count, craftingTable))
      {
        return recipe;
      }
    }
  ...
  function requirementsMetForRecipe(recipe, count, craftingTable) {
    ...
    for (i = 0; i < count; ++i) {
      for (id in delta) {
        if ((bot.inventory.count[id] || 0) + delta[id] * count < 0) return false;
      }
    }
  ...
  function setQuickBarSlot(slot) {
    assert.ok(slot >= 0);
    assert.ok(slot < 9);
    bot.inventory.quickBarSlot = slot;
