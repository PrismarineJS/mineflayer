...
function inject (bot, { version, hideErrors }) {
  ...
  async function waitForClickDelay (amount) {
    if (!amount) return
    if (lastWindowClick + amount < Date.now()) {
  ...
  function extendWindow (window) {
    ...
    window.withdraw = async (itemType, metadata, count, waitBetweenClicks = null, cb = null) => {
      const options = {
        window,
        itemType,
        metadata,
    ...
    window.deposit = async (itemType, metadata, count, waitBetweenClicks = null, cb = null) => {
      const options = {
        window: window,
        itemType,
        metadata,
