...
function inject (bot, { hideErrors }) {
  ...
  // whose claim doesn't match its own state. prismarine-item releases without
  // toHashedNotch (PrismarineJS/prismarine-item#184) fall back to toNotch,
  // which the server answers with resyncs rather than a crash here.
  const toClickNotch = bot.registry.protocol.types.HashedSlot && Item.toHashedNotch ? Item.toHashedNotch : Item.toNotch
