...
function inject (bot, { version, hideErrors }) {
  ...
  async function putSelectedItemRange (start, end, window, slot) {
    ...
    while (window.selectedItem) {
      ...
      if (item && item.stackSize !== item.count) { // something to join with
        Promise.all([await clickWindow(item.slot, 0, 0), await once(window, `updateSlot:${item.slot}`)])
        ...
        if (emptySlot === null) { // no room left
          ...
          await Promise.all([clickWindow(emptySlot, 0, 0), await once(window, `updateSlot:${emptySlot}`)])
        }
      }
    }
  ...
  function closeWindow (window) {
    ...
    console.log('I close the window')
  }
