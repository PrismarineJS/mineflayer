...
function inject(bot) {
  ...
  function genericPutSelItemBack(window, firstEmptyInventorySlot, findInventoryItem, slot, cb) {
    ...
    function tryToFindEmpty() {
      var emptySlot = firstEmptyInventorySlot.call(window);
      if (emptySlot == null) {
    ...
    function tryToJoin() {
      ...
      var item = findInventoryItem.call(window, window.selectedItem.type,
  ...
  function openFurnace(furnaceBlock) {
    ...
    function takeSomething(item, cb) {
      assert.ok(item);
      clickWindowConfirm(item.slot, 0, true, function(err) {
        if (err) {
          cb(err);
        } else {
          cb(null, item);
    ...
    function putSomething(destSlot, itemType, metadata, count, cb) {
      var window = furnace.window;
      assert.ok(window);
      count = count == null ? 1 : count;
      cb = cb || noop;
      var originalSourceSlot = null;
      var destMetadata = metadata;
      putOne();
      ...
      function putOne() {
        ...
        if (!window.selectedItem || window.selectedItem.type !== itemType ||
          (destMetadata != null && window.selectedItem.metadata !== destMetadata))
        {
          // we are not holding the item we need. click it.
          sourceItem = window.findInventoryItem(itemType, metadata);
          if (!sourceItem) return cb(new Error("missing item"));
          if (originalSourceSlot == null) originalSourceSlot = sourceItem.slot;
          clickWindow(sourceItem.slot, 0, false);
          destMetadata = sourceItem.metadata;
        } else {
          sourceItem = window.selectedItem;
          destMetadata = window.selectedItem.metadata;
        }
        var destItem = window.slots[destSlot];
        if (destItem && (destItem.type !== sourceItem.type ||
              destItem.metadata !== destMetadata))
        {
          cb(new Error("item in the way"));
          return;
        }
  ...
  function openChest(chestBlock) {
    ...
    function deposit(itemType, metadata, count, cb) {
      ...
      function depositOne() {
        ...
        var destItem = window.findChestItem(itemType, destMetadata, true);
    ...
    function withdraw(itemType, metadata, count, cb) {
      ...
      function withdrawOne() {
        ...
        var destItem = window.findInventoryItem(itemType, destMetadata, true);
