...
function inject (bot, { hideErrors }) {
  ...
  async function activateEntityWithOptions (entity, position, mouse) {
    // TODO: tell the server that we are not sneaking while doing this
    await bot.lookAt(position, false)
    bot._client.write('use_entity', {
      target: entity.id,
      mouse: mouse, // interact with entity at
      sneaking: false,
      hand: 0, // interact with the main hand
      x: position.x - entity.position.x,
      y: position.y - entity.position.y,
  ...
  bot.activateEntityWithOptions = activateEntityWithOptions
  bot.consume = consume
