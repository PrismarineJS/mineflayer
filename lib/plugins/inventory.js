...
function inject (bot, { version, hideErrors }) {
  ...
  function expectTradeUpdate (window, oldItem) {
    const trade = window.selectedTrade
    const hasOldItem = !!oldItem
    if (hasOldItem !== tradeMatch(trade.inputItem1, window.slots[0])) {
      if (trade.hasItems2) {
        return hasOldItem !== tradeMatch(trade.inputItem2, window.slots[1])
      }
      return true
    }
    return false
  ...
  async function waitForWindowUpdate (window, slot, oldItem) {
    if (window.type === 'minecraft:inventory') {
      ...
      const toUpdate = []
      if (slot <= 2 && !window.selectedTrade.tradeDisabled && expectTradeUpdate(window, oldItem)) {
  ...
  async function clickWindow (slot, mouseButton, mode) {
    ...
    if (!window.transactionRequiresConfirmation(click) && bot.supportFeature('transactionPacketExists')) {
      ...
      await waitForWindowUpdate(window, slot, oldItem)
    }
  }
