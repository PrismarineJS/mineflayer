...
const { sleep, withTimeout } = require('../promise_utils')
...
function inject (bot, { hideErrors }) {
  ...
  let eatingAction = actionCreateDone()
  ...
  bot._client.on('entity_status', (packet) => {
    if (packet.entityId === bot.entity.id && packet.entityStatus === 9 && !eatingAction.done) {
      eatingAction.finish()
    }
    bot.usingHeldItem = false
  })
  ...
  async function consume () {
    if (!eatingAction.done) {
      eatingAction.cancel(new Error('Consuming cancelled due to calling bot.consume() again'))
    }
    ...
    eatingAction = bot.actionCreateCompatible(actions.inventory_consume)

    activateItem()

    await withTimeout(eatingAction.promise, CONSUME_TIMEOUT)
  }
