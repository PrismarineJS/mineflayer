...
function inject (bot, { version, hideErrors }) {
  ...
  async function fish () {
    let bobberDestroyed, getBobber, onParticles

    async function onFishingDone () {
      if (bot._fishingBobber) await activateItem()
      // remove bot._fishingBobber
      delete bot._fishingBobber
      // remove listeners
      bot._client.removeListener('spawn_entity', getBobber)
      bot._client.removeListener('entity_destroy', bobberDestroyed)
      bot._client.removeListener('world_particles', onParticles)
    }

    // const onFishingDone = async () => {
    //   // pull back bobber
    ...
    //   delete bot._fishingBobber
    //   // remove listeners
    //   bot._client.removeListener(bobberDestroyed)
    //   bot._client.removeListener(getBobber)
    //   bot._client.removeListener(onParticles)
    // }
    // function
    async function startFishing (resolve, reject) {
      // functions which use internal variables
      getBobber = makeGetBobber()
      bobberDestroyed = makeBobberDestroyed(reject)
      onParticles = makeOnParticles(resolve)

      // code that is executed
      bot._client.on('spawn_entity', getBobber) // bobber spawned and set to bot._fishingBobber
      await activateItem() // throw bobber
      bot._client.on('entity_destroy', bobberDestroyed)
    ...
    bot._fishing = createPromiseTask(startFishing, { timeout: 60 * 1000 }, onFishingDone)
    return bot._fishing
  }

  function makeGetBobber () {
    return ({ entityId, type }) => { // global so it can be removed after
      if (type === bobberId && !bot._fishingBobber) {
        bot._fishingBobber = bot.entities[entityId]
      }
    }
  }

  function makeBobberDestroyed (reject) {
    return ({ entityIds }) => {
      if (!bot._fishingBobber) return
      if (entityIds.some(id => id === bot._fishingBobber.id)) {
        bot._fishingBobber = undefined
        reject(new Error('Fishing cancelled'))
      }
    }
  }

  function makeOnParticles (resolve) {
    return ({ particleId, particles, x, z }) => {
      if (!bot._fishingBobber) return
      const pos = bot._fishingBobber.position
      if (particleId === 4 && particles === 6 && pos.distanceTo(new Vec3(x, pos.y, z)) <= 1.23) {
        resolve()
      }
    }
  }
...
class Task extends Promise {
  constructor (handler) {
    ...
    this.cancel = cancel
    // only needed if we want to know if promise has been resolved:
...
function createPromiseTask (handler, opts, onComplete) {
  function taskHandler (res, rej) {
    if (opts?.timeout) setTimeout(rej, opts?.timeout)
    handler(res, rej)
  }
  return (new Task(taskHandler)).then(onComplete, onComplete)
}
