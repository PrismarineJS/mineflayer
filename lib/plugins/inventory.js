...
function inject(bot) {
    ...
    function transferOne() {
      ...
      function clickDest() {
        ...
        if (destStart === -999) {
          ...
          console.log("find non full item.", destStart, destEnd, "selected item", window.selectedItem);
          destItem = window.findItemRange(destStart, destEnd,
              window.selectedItem.itemType, window.selectedItem.metadata, true);
  ...
  function confirmTransaction(windowId, actionId, accepted) {
    ...
    console.log("confirmTransaction", "windowId", windowId, "actionId", actionId, "accepted", accepted);
  ...
  function equip(item, destination, cb) {
    ...
    if (item.slot === destSlot) {
      ...
      console.log("looks already equipped to me");
      cb();
      return;
    }

    if (destSlot >= 36 && item.slot >= 36) {
      // all we have to do is change the quick bar selection
      bot.setQuickBarSlot(item.slot - 36);
      console.log("ok just setting quick bar slot.");
      cb();
      return;
    }
    ...
    var options = {
      ...
      count: 1,
      sourceStart: bot.inventory.inventorySlotStart,
      sourceEnd: bot.inventory.inventorySlotStart + windows.INVENTORY_SLOT_COUNT,
      destStart: destSlot,
    };
    transfer(options, cb);
  }
