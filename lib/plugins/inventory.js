...
function inject (bot, { version }) {
  ...
  function transfer (options, cb) {
    ...
    const item = mcData.findItemOrBlockById(itemType)
    ...
    function transferOne () {
      ...
      function clickDest () {
        ...
        const stackSize = item && item.stackSize ? item.stackSize : 64
        const movedItems = Math.min(stackSize - destSlotCount, sourceItemCount)
