  ...
  async function transfer (options) {
    ...
    async function transferOne () {
      ...
      if (!window.selectedItem || window.selectedItem.type !== itemType ||
        ...
        assert(mcData.itemsArray.find(x => x.id === itemType), 'Invalid itemType')
        if (!sourceItem) throw new Error(`Can't find ${mcData.itemsArray.find(x => x.id === itemType).name} in slots [${sourceStart} - ${sourceEnd}], (item id: ${itemType})`)
