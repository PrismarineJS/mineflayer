...
function inject(bot) {
  ...
  function putSelectedItemRange(start, end, window, slot, cb) {
    ...
    function tryToJoin() {
      ...
      if (item) {
        clickWindow(item.slot, 0, false, onClick);
      } else {
        tryToFindEmpty();
      }
    ...
    function tryToFindEmpty() {
      ...
      if (emptySlot == null) {
        ...
        if (slot == null) {
          ...
          clickWindow(slot, 0, false, tossLeftover);
        }
      } else {
        clickWindow(emptySlot, 0, false, cb);
      }
    }
    function tossLeftover() {
      if (window.selectedItem) {
        clickWindow(-999, 0, false, cb);
      } else {
        cb();
      }
    }
  ...
  function openFurnace(furnaceBlock) {
    ...
    bot.client.on(0x69, onUpdateWindowProperty);
    furnace.once("close", onClose);
    return furnace;
    function onClose() {
      bot.client.removeListener(0x69, onUpdateWindowProperty);
    }
    ...
    function takeSomething(item, cb) {
      assert.ok(item);
      clickWindow(item.slot, 0, true, function(err) {
        if (err) {
          cb(err);
        } else {
          cb(null, item);
  ...
  function transfer(options, cb) {
    ...
    function transferOne() {
        ...
        (metadata != null && window.selectedItem.metadata !== metadata))
      {
        ...
        clickWindow(sourceItem.slot, 0, false, function(err) {
          if (err) {
            cb(err);
          } else {
            clickDest();
      ...
      function clickDest() {
        ...
        clickWindow(destSlot, 1, false, function(err) {
          if (err) {
            cb(err);
          } else {
            count -= 1;
  ...
  function openChest(chestBlock) {
    assert.ok(chestBlock.type === 54 || chestBlock.type === 130);
    ...
    function startClicking(window, w, h) {
      ...
      function clickShape() {
             ...
             window.selectedItem.metadata !== ingredient.metadata))
        {
          ...
          if (firstSourceMetadata == null) firstSourceMetadata = sourceItem.metadata;
          clickWindow(sourceItem.slot, 0, false, function(err) {
      ...
      function nextIngredientsClick() {
           ...
           window.selectedItem.metadata !== ingredient.metadata))
        {
          ...
          var sourceItem = window.findInventoryItem(ingredient.id, ingredient.metadata);
          if (!sourceItem) return cb(new Error("missing ingredient"));
          clickWindow(sourceItem.slot, 0, false, clickDest);
        } else {
          clickDest();
        }
        function clickDest() {
      ...
      function grabResult() {
        ...
        // if metadata is null, inshape is guaranteed to be 1x1,
        // copy input metadata
        var outMetadata = recipe.metadata;
        if (outMetadata == null) {
          assert.strictEqual(recipe.inShape.length, 1);
          assert.strictEqual(recipe.inShape[0].length, 1);
          outMetadata = firstSourceMetadata;
        }
        var item = new Item(recipe.type, recipe.count, outMetadata);
        window.updateSlot(0, item);
        // shift click result
        clickWindow(0, 0, true, function (err) {
          if (err) {
            cb(err);
          } else {
            updateOutShape();
      ...
      function updateOutShape() {
        ...
        function next() {
          ...
          clickWindow(theSlot, 0, true, function(err) {
            if (err) {
              cb(err);
            } else {
              next();
  ...
  function clickWindow(slot, mouseButton, shift, cb) {
    cb = cb || noop;
  ...
  function tossStack(item, cb) {
    cb = cb || noop;
    assert.ok(item);
    clickWindow(item.slot, 0, false, function(err) {
      if (err) return cb(err);
      clickWindow(-999, 0, false, cb);
  ...
  function equipEmpty(cb) {
    ...
    var equipSlot = 36 + bot.quickBarSlot;
    clickWindow(equipSlot, 0, false, function(err) {
      if (err) return cb(err);
      clickWindow(slot, 0, false, function(err) {
        if (err) return cb(err);
  ...
  function disrobe(destination, cb) {
    ...
    clickWindow(destSlot, 0, true, function(err) {
      if (err) return cb(err);
      closeWindow(bot.currentWindow || bot.inventory);
    });
  }
  ...
  function moveSlotItem(sourceSlot, destSlot, cb) {
    clickWindow(sourceSlot, 0, false, function(err) {
      if (err) return cb(err);
      clickWindow(destSlot, 0, false, function(err) {
        // if we're holding an item, put it back where the source item was.
        // otherwise we're done.
        if (err) {
          cb(err);
        } else if (bot.inventory.selectedItem) {
          clickWindow(sourceSlot, 0, false, cb);
        } else {
          cb();
        }
      });
