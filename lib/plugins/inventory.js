...
function inject (bot, { hideErrors }) {
  ...
  // The server only sends the cursor when its view of the client is already
  // replaced, e.g. after a rejected click, so applying it never fights a
  // prediction.
  function setCursor (item) {
    const window = bot.currentWindow || bot.inventory
    window.selectedItem = Item.fromNotch(item)
  }
  bot._client.on('set_cursor_item', (packet) => setCursor(packet.contents))
  bot._client.on('set_slot', (packet) => {
    // The cursor's window id is -1, or 255 where the id is a varint
    if (packet.windowId === -1 || packet.windowId === 255) return setCursor(packet.item)
