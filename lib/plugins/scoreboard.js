...
function inject (bot) {
  ...
  bot._client.on('scoreboard_objective', (packet) => {
    ...
    if (packet.action === 2) {
      ...
      bot.emit('scoreboardTitleUpdated', scoreboards[packet.name])
    }
  })
  ...
  bot._client.on('scoreboard_score', (packet) => {
    ...
    if (packet.action === 0) {
      ...
      bot.emit('scoreboardScoreUpdated', scoreboard, updated)
    }

    if (packet.action === 1) {
      const removed = scoreboard.remove(packet.itemName)
      bot.emit('scoreboardScoreRemoved', scoreboard, removed)
    }
  })
  ...
  bot._client.on('scoreboard_display_objective', (packet) => {
    ...
    if (scoreboard) {
      bot.emit(`updateScoreboardPosition`, position, scoreboard, ScoreBoard.positions[position])
