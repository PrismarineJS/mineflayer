...
function inject (bot, options) {
  ...
  bot._client.on('player_chat', (packet) => {
    ...
    let verified = false
    const { profileKeys } = bot.players[bot.uuidToUsername[packet.senderUuid]]
  ...
  bot.awaitMessage2 = (opts) => {
    if (!bot.registry.supportFeature('clientboundChatHasSender')) {
      // Older versions don't send sender as a seperate field. So we must use regex to deduce the sender.
      // The regex depends on the server software, so it's set to vanilla by default but can be modified
      // with `legacyMessageRegex` option.
      if (!opts.legacyMessageRegex) opts.legacyMessageRegex = LEGACY_VANILLA_CHAT_REGEX
    } else if (!opts.senderUUID && opts.senderName) {
      opts.senderUUID = bot.players[opts.senderName]?.uuid
    }
    if (opts.containing && !Array.isArray(opts.containing)) opts.containing = [opts.containing]

    return new Promise((resolve, reject) => {
      function onMessage (message, chatPosition, msg, sender, verified) {
        let senderMatch
        let messageContent = msg.content?.text

        if (sender === undefined) { // versions pre-1.16
          const m = message.match(opts.legacyMessageRegex)
          const senderName = m?.[1].trim()
          const msg = m?.[2].trim()

          if (opts.senderName && senderName === opts.senderName) {
            senderMatch = true
          }
          if (msg) messageContent = msg
        } else if (opts.senderUUID) {
          senderMatch = opts.senderUUID === sender
        } else {
          senderMatch = true // not matching by sender
        }

        // check and resolve if a hit
        const contentMatch = opts.containing?.some(x => x instanceof RegExp ? x.test(messageContent) : messageContent.includes(x)) ?? true
        const verifiedMatch = opts.verified ? verified : true
        const positionMatch = opts.position ? chatPosition === opts.position : true
        // console.log('contentMatch', contentMatch, senderMatch, verifiedMatch)
        if (contentMatch && senderMatch && verifiedMatch && positionMatch) {
          resolve({ sender, message, rawMessage: msg })
      ...
      if (opts.timeout) {
        setTimeout(() => reject(Error('timed out')), opts.timeout)
      }
    })
  }
