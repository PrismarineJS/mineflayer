...
async function awaitMessage (awaitMessageTimeoutTime, ...args) {
  if (awaitMessageTimeoutTime === undefined || args === undefined) throw new Error('Missing arguments!')
  if (Number.isSafeInteger(Number(awaitMessageTimeoutTime)) != true) throw new Error('awaitMessageTimeoutTime must be an integer!')
  awaitMessageTimeoutTime = Number(awaitMessageTimeoutTime)
  return new Promise((resolve, reject) => {
    const resolveMessages = args.flatMap(x => x)
    function messageListener (msg) {
      if(resolveMessages.some(x => x instanceof RegExp ? x.test(msg) : msg === x)) {
        clearTimeout(timeoutAwaitMessage)
        bot.off('messagestr', messageListener)
        resolve(msg)
      }
    }
    const timeoutAwaitMessage = setTimeout(async () => {
      bot.off('messagestr', messageListener)
      reject(new Error('awaitMessage timed out!'))
    }, awaitMessageTimeoutTime)
