...
function inject(bot) {
  ...
  bot._client.on('chat', function(packet) {
    ...
    // Function used to process "unidentified" messages, such as chat and from bikkit servers
    function parseOldMessage(message) {
      var legalContent = message.replace(incomingFilter, '');
      var match, username, content;

			// iterate through each object in chat.patterns arrayand test if .pattern matches
