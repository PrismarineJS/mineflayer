...
function inject (bot, options) {
  ...
  bot._client.on('playerChat', (data) => {
    ...
    if (bot.supportFeature('clientsideChatFormatting')) {
      ...
      if (message) content = JSON.parse(message)
      else if (options.allowInsecureChat && data.unsignedContent) content = JSON.parse(data.unsignedContent)
      ...
      try {
        ...
        sender = {
          ...
          hoverEvent: { action: 'show_entity', contents: { id: bot.findPlayer(data.senderName).id /* TODO: Get entity ID of player */, name: data.senderName }},
  ...
  function chatWithHeader (header, message) {
    ...
    if (bot.supportFeature('signedChat')) {
      if (message.startsWith('/')) {
        ...
        if (bot.supportFeature('chainedChatWithHashing')) {
