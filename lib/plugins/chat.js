...
function awaitMessage (...args) {
  return new Promise((resolve, reject) => {
    if(!args[1]) reject(new Error('Missing argument! timeoutAwaitMessageTime'));
    if(Number.isSafeInteger(Number(args[1])) != true) reject(new Error('timeoutAwaitMessageTime must be an integer!'));
    const timeoutAwaitMessage = setTimeout(async () => {
      reject('No matching message found in ' + Number(args[1])/1000 + ' second(s).')
    }, Number(args[1]))
    const resolveMessages = new Array(args[0]).flatMap(x => x)
