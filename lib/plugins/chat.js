...
function inject(bot) {
  bot.client.on('chat', function(packet) {
    ...
    function parseJsonMessage7(jsonMessage) {
      ...
      // Emit just the message so plugins can handle achievements and other things in the future
      // Optionally we could use a 'chat' event and pad out the arguments we don't know.
