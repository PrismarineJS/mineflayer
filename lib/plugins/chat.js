  ...
  bot.client.on('chat', function(packet) {
    ...
    function parseJsonMessage(jsonMessage) {
      ...
      if (typeof jsonMsg.translate === 'string' && jsonMsg.translate.match(/^chat\./)) {
        // spoken chat
        if(jsonMsg.using && Array.isArray(jsonMessage.using)) {
          username = jsonMsg.using[0];
          content = jsonMsg.using[1].replace(incomingFilter, '');
        }else{
          var message = new ChatMessage(jsonMsg.translate, jsonMsg.with);
          username = message.getText(0);
          content = message.toString().substring(username.length).trim();
        }
        bot.emit('chat', username, content, jsonMsg.translate, message)
