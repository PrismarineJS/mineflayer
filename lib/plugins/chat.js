...
function inject(bot) {
  bot.client.on(0x03, function(packet) {
    ...
    if (match) {
      ...
      var content = match[2];
      var legalContent = content.replace(legalRe, '');
	  var legalContent = content.replace(new RegExp("§.", 'g'), '');
      bot.emit('chat', username, legalContent, message);
    } else {
      // non-spoken chat
      var legalMsg = message.replace(legalRe, '');
	  var legalMsg = message.replace(new RegExp("§.", 'g'), '');
