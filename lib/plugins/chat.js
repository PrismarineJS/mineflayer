...
function inject (bot, options) {
  ...
  async function findMessage (findMessageTimeout, ...args) {
    if (findMessageTimeout === undefined || args === undefined) {
      throw new Error('Missing arguments!')
    }
    if (typeof Number(findMessageTimeout) !== 'number') {
      throw new Error('Invalid findMessageTimeout value! Value must be an integer.')
    }
    findMessageTimeout = Number(findMessageTimeout)
    let findMessageResult
    const findMessagePromise = new Promise((resolve, reject) => {
      const resolveMessages = args.flatMap(x => x)
      function messageListener (msg) {
        if (resolveMessages.some(x => x instanceof RegExp ? x.test(msg) : msg === x)) {
          clearTimeout(timeoutFindMessage)
          bot.off('messagestr', messageListener)
          resolve(msg)
        }
      }
      const timeoutFindMessage = setTimeout(async () => {
        bot.off('messagestr', messageListener)
        reject(new Error(`findMessage timed out after ${findMessageTimeout} milliseconds!`))
      }, findMessageTimeout)
      bot.on('messagestr', messageListener)
    })
    await findMessagePromise.then(messageFound => {
      findMessageResult = messageFound
    }, () => {
      findMessageResult = false
    })
    return findMessageResult
  }
  bot.findMessage = findMessage
}
