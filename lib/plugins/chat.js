...
const USERNAME_REGEX = '(?:\\(.{1,16}\\)|\\[.{1,16}\\]|.){0,5}?(\\w+)'
...
function inject (bot, options) {
  ...
  async function onMessage (onMessageTimeout, ...args) {
    if (onMessageTimeout === undefined || args === undefined) {
      throw new Error('Missing arguments!')
    }
    if (typeof Number(onMessageTimeout) !== 'number') {
      throw new Error('Invalid onMessageTimeout value! Value must be an integer.')
    }
    onMessageTimeout = Number(onMessageTimeout)
    return new Promise((resolve, reject) => {
      const resolveMessages = args.flatMap(x => x)
      function messageListener (msg) {
        if (resolveMessages.some(x => x instanceof RegExp ? x.test(msg) : msg === x)) {
          resolve(msg)
          bot.off('messagestr', messageListener)
          clearTimeout(timeoutOnMessage)
        }
      }
      const timeoutOnMessage = setTimeout(async () => {
        resolve(false)
        bot.off('messagestr', messageListener)
      }, onMessageTimeout)
      bot.on('messagestr', messageListener)
