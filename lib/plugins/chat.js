...
function inject (bot, options) {
  ...
  function onChat (packet) {
    ...
    let verified
    if (packet.signature) {
      verified = bot._client.verifyMessage(bot.players[packet.senderUuid].crypto.publicKey, packet)
    } else {
      verified = null
    }
    const msg = ChatMessage.fromNotch(message)
    ...
    const chatPosition = packet.position === undefined ? undefined : ChatPositions[packet.position]

    bot.emit('message', msg, chatPosition, verified)
    bot.emit('messagestr', msg.toString(), chatPosition, msg, verified)

    // Position 2 is the action bar
    if (packet.position === 2) bot.emit('actionBar', msg, verified)
  }

  bot._client.on('chat', onChat)
  bot._client.on('player_chat', onChat)
      ...
      for (i = 0; i < subMessage.length; i += lengthLimit) {
        ...
        if (bot.supportFeature('signedChat')) {
          ...
          bot._client.write('chat_message', {
            ...
            timestamp,
            salt: 0,
            signature: bot._client.signMessage(smallMsg, timestamp)
          })
        } else {
