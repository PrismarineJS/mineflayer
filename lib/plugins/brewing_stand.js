...
function inject (bot) {
  ...
  async function openBrewingStand (brewingStandBlock) {
    ...
    const [fuelPacket] = await onceWithCleanup(bot._client, 'craft_progress_bar', {timeout: 2500, checkCondition: (packet) => (packet.property === 1)})
    ...
    brewingStand.fuel = fuelPacket.value
    ...
    function storeGuiFuel (packet) {
      if (packet.property !== 1) return
      tempFuel = packet.value
    }
