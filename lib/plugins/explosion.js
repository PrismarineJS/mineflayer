...
function calcExposure (entity, explosionPos, world) {
  ...
  for (pos.y = entity.position.y; pos.y <= entity.position.y + height; pos.y += height * dy) {
    for (pos.x = entity.position.x - halfWidth + d3; pos.x <= entity.position.x + halfWidth; pos.x += width * dx) {
      for (pos.z = entity.position.z - halfWidth + d4; pos.z <= entity.position.z + halfWidth; pos.z += width * dz) {
        dir.set(pos.x - explosionPos.x, pos.y - explosionPos.y, pos.z - explosionPos.z)
...
function inject (bot) {
  ...
  const difficultyValues = { peaceful: 0, easy: 1, normal: 2, hard: 3 }
