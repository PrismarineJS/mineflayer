...
function inject(bot,{version}) {
    ...
    function gotTrades(packet) {
      ...
      villager.trades = packet.trades.map(function (trade) {
        return Object.assign(trade, {
          ...
          secondaryInput: Item.fromNotch(trade.secondaryInput || {blockId: -1}),
  ...
  function putRequirements(window, Trade, cb) {
    deposit(window, Trade.firstInput.type, Trade.firstInput.metadata, Trade.firstInput.count, 0, function (err) {
      if (err) {
        cb(err);
      } else if (Trade.hasSecondItem) {
        deposit(window, Trade.secondaryInput.type, Trade.secondaryInput.metadata, Trade.secondaryInput.count, 1, function (err) {
          if (err) {
            cb(err);
          } else {
            cb();
