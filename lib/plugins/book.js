...
function inject (bot) {
  ...
  async function write (slot, pages, author, title, signing) {
    ...
    let movedToQuickBar = false
    let changedQuickBarSlot = false
    try {
      if (moveToQuickBar) {
        await bot.moveSlotItem(slot, 36)
        movedToQuickBar = true
      ...
      if (changedQuickBarSlot) bot.setQuickBarSlot(quickBarSlot)
      if (movedToQuickBar) await bot.moveSlotItem(36, slot)
    }
  }
