...
function inject (bot) {
  ...
  const hostileMobs = new Set(['blaze', 'creeper', 'drowned', 'endermite', 'enderman', 'evoker', 'giant', 'guardian', 'elder_guardian',
    'illusioner', 'pillager', 'phantom', 'ravager', 'silverfish', 'skeleton', 'stray', 'wither_skeleton', 'spider', 'cave_spider', 'vex',
    'vindicator', 'witch', 'Wither', 'zombie', 'zombie_villager', 'husk'])
  ...
  function parseBedMetadata (bedBlock) {
    ...
    if (bot.majorVersion === '1.13') {
      ...
      const bitMetadata = state.toString(2).padStart(4, '0') // FACING (2 bits) | PART (1 bit) | OCCUPIED (1 bit)
      metadata.part = !parseInt(bitMetadata[3], 2)
      metadata.occupied = !parseInt(bitMetadata[2], 2)
      ...
      const bitMetadata = bedBlock.metadata.toString(2).padStart(4, '0') // PART (1 bit) | OCCUPIED (1 bit) | FACING (2 bits)
      metadata.part = Boolean(parseInt(bitMetadata[0], 2))
      metadata.occupied = Boolean(parseInt(bitMetadata[1], 2))
  ...
  function sleep (bedBlock, cb) {
    ...
    if (!(bot.time.day >= 12541 && bot.time.day <= 23458)) {
      ...
      if (!(bot.majorVersion === '1.13' && bot.gameMode === 'creative')) { // If in creative mode the bot should be able to sleep even if there are monster nearby (starting in 1.13)
        ...
        for (const key of Object.keys(bot.entities)) {
          ...
          if (entity.kind === 'Hostile mobs' || hostileMobs.has(String(entity.name).toLowerCase())) {
