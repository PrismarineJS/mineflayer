  ...
  async function sleep (bedBlock) {
    ...
    if (!thunderstorm && !(bot.time.timeOfDay >= 12541 && bot.time.timeOfDay <= 23458)) {
      ...
      if (!metadata.part) { // Is foot
        const upperBlock = bot.world.getBlock(bedBlock.position.plus(metadata.headOffset))

        if (isABed(upperBlock)) {
          headPoint = upperBlock.position
        } else {
          const lowerBlock = bot.world.getBlock(bedBlock.position.plus(metadata.headOffset.scaled(-1)))
