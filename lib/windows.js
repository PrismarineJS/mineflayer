...
Window.prototype.findItemRange = function(start, end, itemType, metadata, notFull) {
  ...
  for (var i = start; i < end; ++i) {
    ...
    if (i === start) {
      console.log("findItemRange - checking 1st slot", "start", start, "end", end);
      if (item) {
        console.log("findItemRange - item OK");
        if (itemType === item.type) {
          console.log("findItemRange - itemType === item.type OK");
        } else {
          console.log("findItemRange - [BAD]", "itemType", itemType, "item.type", item.type);
        }
        if (metadata == null || metadata === item.metadata) {
          console.log("findItemRange - metadata OK");
        } else {
          console.log("findItemRange - [BAD]", "metadata", metadata, "item.metadata", item.metadata);
        }
        if (!notFull || item.count < item.stackSize) {
          console.log("findItemRange - notFull OK");
        } else {
          console.log("findItemRange - [BAD]", "item.count", item.count, "item.stackSize", item.stackSize);
        }
      } else {
        console.log("findItemRange - [BAD] item is null");
      }
    }
...
Window.prototype.acceptNonInventorySwapAreaClick = function(click) {
  if (click.shift) {
    this.acceptNonInventorySwapAreaShiftClick(click);
  } else {
    if (click.mouseButton === 0) {
      this.acceptSwapAreaLeftClick(click);
    } else if (click.mouseButton === 1) {
...
Window.prototype.acceptNonInventorySwapAreaShiftClick = function(click) {
  assert.strictEqual(click.mouseButton, 0);
  assert.strictEqual(click.shift, true);
...
Window.prototype.acceptSwapAreaRightClick = function(click) {
  ...
  assert.strictEqual(click.shift, false);
...
Window.prototype.transactionRequiresConfirmation = function(click) {
  return true;
  var inventorySlotEnd = this.inventorySlotStart + INVENTORY_SLOT_COUNT;
  if (click.slot >= this.inventorySlotStart && click.slot < inventorySlotEnd) {
    if (this.selectedItem || click.shift) {
      return true;
    } else {
      return false;
    }
...
InventoryWindow.prototype.acceptCraftingClick = function(click) {
  this.acceptNonInventorySwapAreaShiftClick(click);
...
CraftingTableWindow.prototype.acceptCraftingClick = function(click) {
  this.acceptNonInventorySwapAreaShiftClick(click);
