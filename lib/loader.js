...
const { supportedVersions, testedVersions, latestSupportedVersion, oldestSupportedVersion } = require('./version')
const oldestSupportedVersionData = minecraftData.versionsByMinecraftVersion.pc[oldestSupportedVersion]
const latestSupportedVersionData = minecraftData.versionsByMinecraftVersion.pc[latestSupportedVersion]
...
function createBot (options = {}) {
  ...
  function next () {
    ...
    if (versionData.version > latestSupportedVersionData.version) {
      throw new Error(`Server version '${serverPingVersion}' is not supported. Latest supported version is '${latestSupportedVersion}'.`)
    } else if (versionData.version < oldestSupportedVersionData.version) {
