...
function createBot (options = {}) {
  ...
  function next () {
    const versionStrFromServerPing = bot._client.version
    bot.registry = require('prismarine-registry')(versionStrFromServerPing)
    if (!bot.registry?.version) throw new Error(`Version '${versionStrFromServerPing}' is not supported, no data for version`)

    const versionData = bot.registry.version
    const latestTestedMineflayerVersion = testedVersions[testedVersions.length - 1]
    const latestTestedVersionData = minecraftData.versionsByMinecraftVersion.pc[latestTestedMineflayerVersion]
    if (versionData.version > latestTestedVersionData.version) {
      throw new Error(`Version '${versionStrFromServerPing}' is not supported. Latest supported version is '${latestTestedMineflayerVersion}'.`)
    }
