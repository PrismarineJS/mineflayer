...
function onceWithCleanup (emitter, event, { timeout = 0, checkCondition = undefined } = {}) {
  ...
  const onEvent = (...data) => {
    if (checkCondition && !checkCondition(...data)) {
      return
    }
  ...
  if (timeout > 0) {
    ...
    const timeoutError = new Error(`Event ${event} did not fire within timeout of ${timeout}ms ${checkCondition && checkCondition.name}`)
    sleep(timeout).then(() => {
      if (!task.done) {
...
function withTimeout (promise, timeout) {
  return new Promise((resolve, reject) => {
    const timeoutHandle = setTimeout(() => reject(new Error('Promise timed out.')), timeout)
    promise
      .finally(() => {
