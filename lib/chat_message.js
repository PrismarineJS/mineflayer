...
var lang = {}
fs.readFileSync('en_us.lang','utf8').split('\n').forEach(function (l) {
  var c = l.split(/=(.+)/);
...
ChatMessage.prototype.toMotd = function(motd) {
  var codes = {
    color:{
      black: '§0',
      dark_blue: '§1',
  ...
  if(this.extra) {
    this.extra.forEach(function(entry) {
      message += entry.toMotd() + " ";
    })
  }
  return message.trim();
};

ChatMessage.prototype.toAnsi = function(motd) {
  var codes = {
    '§0': '\u001b[30m',
    '§1': '\u001b[34m',
    '§2': '\u001b[32m',
