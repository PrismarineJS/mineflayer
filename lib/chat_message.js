...
ChatMessage.prototype.parse = function() {
  ...
  if(typeof json.text === 'string') {
    ...
    if(typeof json.with === 'object') {
      ...
      this.with = json.with.map(function(entry) {
        return new ChatMessage(entry, that.lang);
      });
    }
  }
  ...
  if(typeof json.extra === 'object') {
    ...
    this.extra = json.extra.map(function(entry) {
      return new ChatMessage(entry, that.lang);
    });
  }
  // Text modifiers
 ...
 * @return {String}
 */
ChatMessage.prototype.getText = function(idx, lang) {
  lang = lang || this.lang;
 ...
 * @return {String}
 */
ChatMessage.prototype.toString = function(lang) {
  lang = lang || this.lang;
  var message = "";
  if(typeof this.text === 'string') message += this.text + " ";
  else if(this.with) {
    var args = this.with.map(function(entry) {
      return entry.toString(lang);
    })
    var format = lang[this.translate]
    if (!format) message += args.join(" ") + " ";
    else message += vsprintf(format, args) + " ";
  }
  if(this.extra) {
    this.extra.forEach(function(entry) {
      message += entry.toString(lang) + " ";
    })
  }
  return message.replace(/§[0-9a-flnmokr]/g, "").trim();
};

ChatMessage.prototype.toMotd = function(lang) {
  lang = lang || this.lang;
  var codes = {
    color:{
      black: '§0',
  ...
  var message = Object.keys(codes).map((function (code) {
  ...
  if(this.extra) {
    this.extra.forEach(function(entry) {
      message += entry.toMotd(lang) + " ";
    })
  }
  return message.trim();
};

ChatMessage.prototype.toAnsi = function(lang) {
  var codes = {
    '§0': '\u001b[30m',
    '§1': '\u001b[34m',
    '§2': '\u001b[32m',
