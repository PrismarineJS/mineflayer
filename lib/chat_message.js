...
class ChatMessage {
  ...
  toMotd (lang = defaultLang, parent = {}) {
    ...
    let message = Object.keys(codes).map((code) => {
      self[code] = parent[code] || this[code]
      if (!self[code] || self[code] === 'false') return null
      if (code === 'color') return codes.color[self.color]
      return codes[code]
    }).join('')
    ...
    if (this.extra) {
      message += this.extra.map((entry, index) => {
        return entry.toMotd(lang, this)
      }).join('')
    }
    return message
  }
