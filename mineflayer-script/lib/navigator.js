...
(function() {
    ...
    navigator.navigateTo = function(end, params) {
        ...
        var path = a_star.findPath({
            ...
            "distance_func": function(node_a, node_b) { return node_a.point.distanceTo(node_b.point); },
            "heuristic_func": function(node) { return node.point.distanceTo(end) + 5 * node.water; },
    ...
    function getNeighbors(node) {
        ...
        for (var i = 0; i < cardinal_direction_vectors.length; i++) {
            ...
            var can_jump_past_j = can_jump_forward && block_j.safe && block_i.safe;
            if (can_jump_past_j && block_k.physical) {
            ...
            var can_land_on_l = false;
            if (block_i.safe && block_j.safe && block_k.safe && block_l.physical) {
            ...
            var can_land_on_m = false;
            if (block_e.safe) {
                // can drop through e
                var point_m = pointAt(2, -3);
                var block_m = properties(point_m);
                if (block_j.safe && block_k.safe && block_l.safe && block_m.physical) {
                    // can walk and drop onto m
                    can_land_on_m = true;
                    result.push(point_l);
                }
                var block_n = properties(pointAt(2, -4));
                if (block_k.safe && block_l.safe && block_m.safe && block_n.physical) {
            ...
            if (!can_land_on_l && block_q.safe && block_r.safe && block_s.physical) {
                // can jump way over and down onto s
                result.push(point_r);
                continue;
            }
            var block_t = properties(pointAt(3, -3));
            if (!can_land_on_m && block_r.safe && block_s.safe && block_t.physical) {
